...
if($rowcount==1){
			...
			$pdf->cell(270,5,'SEA/LAND CARGO MANIFEST',0,1,'C');
            ...
            $columnwidths = array(8,15,15,20,40,40,20,20,20,20,20,20,15);


			function printHeader($pdf,$headerlinespacing,$columnwidths){
				$pdf->SetFont('Arial','B',7);

                $pdf->Cell($columnwidths[0],$headerlinespacing,'','LTR',0,'C');
				$pdf->Cell($columnwidths[1],$headerlinespacing,'PICKUP','LTR',0,'C');
				$pdf->Cell($columnwidths[2],$headerlinespacing,'','LTR',0,'C');
				$pdf->Cell($columnwidths[3],$headerlinespacing,'','LTR',0,'C');
				$pdf->Cell($columnwidths[4],$headerlinespacing,'','LTR',0,'C');
                $pdf->Cell($columnwidths[5],$headerlinespacing,'','LTR',0,'C');
                $pdf->Cell($columnwidths[6],$headerlinespacing,'','LTR',0,'C');
				$pdf->Cell($columnwidths[7],$headerlinespacing,'','LTR',0,'C');
				$pdf->Cell($columnwidths[8],$headerlinespacing,'','LTR',0,'C');
                $pdf->Cell($columnwidths[9],$headerlinespacing,'DIMENSIONAL','LBTR',0,'C');
                $pdf->Cell($columnwidths[10],$headerlinespacing,'','LTR',0,'C');
                $pdf->Cell($columnwidths[11],$headerlinespacing,'SHIPPING','LTR',0,'C');
				$pdf->Cell($columnwidths[12],$headerlinespacing,'','LTR',1,'C');
			

				$pdf->Cell($columnwidths[0],$headerlinespacing,'NO.','BLR',0,'C');
				$pdf->Cell($columnwidths[1],$headerlinespacing,'DATE','BLR',0,'C');
				$pdf->Cell($columnwidths[2],$headerlinespacing,'BRF#','BLR',0,'C');
				$pdf->Cell($columnwidths[3],$headerlinespacing,'BOL #','BLR',0,'C');
				$pdf->Cell($columnwidths[4],$headerlinespacing,'SHIPPER\'S NANE','BLR',0,'C');
                $pdf->Cell($columnwidths[5],$headerlinespacing,'CONSIGNEE\'S NANE','BLR',0,'C');
                $pdf->Cell($columnwidths[6],$headerlinespacing,'DESTINATION','BLR',0,'C');
				$pdf->Cell($columnwidths[7],$headerlinespacing,'DESCRIPTION','BLR',0,'C');
				$pdf->Cell($columnwidths[8],$headerlinespacing,'NO. OF PKGS','BLR',0,'C');
                $pdf->Cell($columnwidths[9],$headerlinespacing,'L x W x H / 3500','BLR',0,'C');
                $pdf->Cell($columnwidths[10],$headerlinespacing,'Actual Weight','BLR',0,'C');
                $pdf->Cell($columnwidths[11],$headerlinespacing,'LINE/PLATE #','BLR',0,'C');
				$pdf->Cell($columnwidths[12],$headerlinespacing,'REMARKS','BLR',1,'C');
			}
			...
			if($rscount>0){
				...
				while ($obj = mysql_fetch_object($rs)) {
                    ...
                    $numofpackages = utf8_encode($obj->package_number_of_packages);
                    $cbm = utf8_encode($obj->package_cbm);
                    $actualweight = utf8_encode($obj->package_actual_weight);
                    $platenumber = utf8_encode($obj->plate_number);
                    $status = '';
                    $remarks = '';
