...
if(getNumRows($rs)==1){
	while($obj = fetch($rs)){
		...
		$rs1  = query("SELECT txn_billing_waybill.id,
							...
							ifnull(if(mode_of_transport.id in (select id from mode_of_transport where description like '%SEA%'),concat(format(package_cbm,1),'cbm'),replace(concat(format(package_actual_weight,2),if(package_actual_weight<0,'gms',if(package_actual_weight=1,'kg','kgs'))),'.00','')),'0') weight_display
