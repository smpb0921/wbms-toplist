...
{	
	function Footer()
	{
		...
		$this->SetY(-8);
		$this->Cell(260,3,'',0,0,'L');

	    $this->SetY(-8);
	    $this->SetFont('Arial','B',7);
	    $this->Cell(260,3,'Page '.$this->PageNo().' of {nb}',0,0,'C');
...
if($rowcount==1){
			...
			$pdf->cell(270,5,'COSTING TRANSACTION',0,1,'C');
			...
			$columnwidths = array(10,25,20,20,25,20,20,20,25,25,25,35);
			function printHeader($pdf,$headerlinespacing,$columnwidths){
				...
				$pdf->Cell($columnwidths[4],$headerlinespacing,'TOTAL AMOUNT','TBLR',0,'C');
				$pdf->Cell($columnwidths[5],$headerlinespacing,'FREIGHT COST','TBLR',0,'C');
				$pdf->Cell($columnwidths[6],$headerlinespacing,'AGENT COST','TBLR',0,'C');
				$pdf->Cell($columnwidths[7],$headerlinespacing,'INSURANCE','TBLR',0,'C');
				$pdf->Cell($columnwidths[8],$headerlinespacing,'OTHER EXPENSES','TBLR',0,'C');
				$pdf->Cell($columnwidths[9],$headerlinespacing,'GROSS INCOME','TBLR',0,'C');
				$pdf->Cell($columnwidths[10],$headerlinespacing,'CREATED DATE','TBLR',0,'C');
				$pdf->Cell($columnwidths[11],$headerlinespacing,'CREATED BY','TBLR',1,'C');
			}
