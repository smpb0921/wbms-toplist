...
if($rowcount==1){
			...
			while($obj=fetch($rs)){
				...
				$headerdestination = utfEncode($obj->destination);
			...
			$pdf->cell(270,5,'DISPATCH TRANSMITTAL',0,1,'C');
			...
			$columwidths = array(8,20,20,20,55,50,20,20,25,30);


			function printHeader($pdf,$headerlinespacing,$columwidths){
				$pdf->SetFont('Arial','B',7);

				$pdf->Cell($columwidths[0],$headerlinespacing,'','TLR',0,'C');
				$pdf->Cell($columwidths[1],$headerlinespacing,'','TLR',0,'C');
				$pdf->Cell($columwidths[2],$headerlinespacing,'','TLR',0,'C');
				$pdf->Cell($columwidths[3],$headerlinespacing,'','TLR',0,'C');
				$pdf->Cell($columwidths[4],$headerlinespacing,'','TLR',0,'C');
				$pdf->Cell($columwidths[5],$headerlinespacing,'','TLR',0,'C');
				$pdf->Cell($columwidths[6],$headerlinespacing,'','TLR',0,'C');
                $pdf->Cell($columwidths[7],$headerlinespacing,'','TLR',0,'C');
				$pdf->Cell($columwidths[8],$headerlinespacing,'DAYS','TLR',0,'C');
				$pdf->Cell($columwidths[9],$headerlinespacing,'','TLR',1,'C');
			

				$pdf->Cell($columwidths[0],$headerlinespacing,'NO.','BLR',0,'C');
				$pdf->Cell($columwidths[1],$headerlinespacing,'PICKUP DATE','BLR',0,'C');
				$pdf->Cell($columwidths[2],$headerlinespacing,'BRF#','BLR',0,'C');
				$pdf->Cell($columwidths[3],$headerlinespacing,'BOL #','BLR',0,'C');
				$pdf->Cell($columwidths[4],$headerlinespacing,'SHIPPER\'S NANE','BLR',0,'C');
				$pdf->Cell($columwidths[5],$headerlinespacing,'DESCRIPTION','BLR',0,'C');
				$pdf->Cell($columwidths[6],$headerlinespacing,'TRACKING NO.','BLR',0,'C');
                $pdf->Cell($columwidths[7],$headerlinespacing,'POUCH SIZE','BLR',0,'C');
				$pdf->Cell($columwidths[8],$headerlinespacing,'PROCESSED','BLR',0,'C');
				$pdf->Cell($columwidths[9],$headerlinespacing,'CREATED BY','BLR',1,'C');
			}

			printHeader($pdf,$headerlinespacing,$columwidths);
			...
			$rs = query("
						    select txn_manifest_waybill.id,
                                   ...
                                   concat(user.first_name,' ',user.last_name) as bolcreator
			...
			if($rscount>0){
				...
				while ($obj = mysql_fetch_object($rs)) {
                    ...
                    $mawbl = utf8_encode($obj->mawbl_bl);
                    $pouchsize = utf8_encode($obj->pouchsize);
                    $bolcreator = utf8_encode($obj->bolcreator);
                    $status = '';
                    $remarks = '';

					$ddate = strtotime($docdate);
				...
				//print_r($daysprccount);
						...
						$pdf->Cell(28,8,'TOTAL','TBLR',0,'C');
						$pdf->Cell(185,8,trim($pouchsizedesc,', '),'TBLR',0,'C');
						$pdf->Cell(25,8,$morethanadaycounter,'TBLR',0,'C');
						$pdf->Cell(30,8,'More than 1 day','TBLR',1,'C');

						$pdf->Cell(67,8,"1 day - ".$daysprccount[0]." ".pcsuffix($daysprccount[0]),'TBLR',0,'C');
						$pdf->Cell(67,8,"2 days - ".$daysprccount[1]." ".pcsuffix($daysprccount[1]),'TBLR',0,'C');
						$pdf->Cell(67,8,"3 days - ".$daysprccount[2]." ".pcsuffix($daysprccount[2]),'TBLR',0,'C');
						$pdf->Cell(67,8,"4 days and above - ".$daysprccount[3]." ".pcsuffix($daysprccount[3]),'TBLR',1,'C');
