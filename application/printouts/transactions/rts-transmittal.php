...
{	
	function Footer()
	{
		...
		$this->SetY(-8);
		$this->Cell(135,3,'Date Effective: October 1, 2021',0,0,'L');
		$this->Cell(135,3,'QF-OPR-16 Issue #2 rev.01',0,0,'R');
...
$rs = query("select carrier.description as carrier,
					...
					group_concat(distinct shipper.company_name separator ', ') as shipper
	         from txn_manifest 
...
if($rowcount==1){
			...
			while($obj=fetch($rs)){
				...
				$shipper = utf8_encode($obj->shipper);
			}
			...
			$pdf->cell(270,5,'RTS SHIPMENT TRANSMITTAL',0,1,'C');
			...
			$pdf->SetFont('Arial','',0);
			$pdf->cell(100,5,$docdate,'B',1,'L');

			$pdf->SetFont('Arial','B',9);
			$pdf->cell(35,5,'Company Name: ',0,0);
			$pdf->SetFont('Arial','',9);
			$pdf->cell(100,5,$shipper,'B',1,'L');

			$pdf->SetFont('Arial','B',9);
			$pdf->cell(35,5,'Address:',0,0);
			$pdf->SetFont('Arial','',9);
			$pdf->cell(100,5,'','B',1,'L');
			...
			$pdf->SetFont('Arial','',9);
			$pdf->cell(130,5,'We are submitting the return (RTS) of your shipments, attached the original proof with corresponding reasons. ',0,1);

			$pdf->SetFont('Arial','B',7);
			$headerlinespacing = 5;

			$columnwidths = array(12,26,25,25,30,50,50,30,25);
			function printHeader($pdf,$headerlinespacing,$columnwidths){
				$pdf->SetFont('Arial','B',7);
				

				$pdf->Cell($columnwidths[0],$headerlinespacing,'NO.','BTLR',0,'C');
				$pdf->Cell($columnwidths[1],$headerlinespacing,'DATE PICKUP','BTLR',0,'C');
				$pdf->Cell($columnwidths[2],$headerlinespacing,'BRF#','BTLR',0,'C');
				$pdf->Cell($columnwidths[3],$headerlinespacing,'BOL','BTLR',0,'C');
				$pdf->Cell($columnwidths[4],$headerlinespacing,'SYS GENERATED DR','BTLR',0,'C');
				$pdf->Cell($columnwidths[5],$headerlinespacing,'SHIPPER\'S NANE','BTLR',0,'C');
				$pdf->Cell($columnwidths[6],$headerlinespacing,'CONSIGNEE\'S NAME','BTLR',0,'C');
				$pdf->Cell($columnwidths[7],$headerlinespacing,'DESCRIPTION','BTLR',0,'C');
				$pdf->Cell($columnwidths[8],$headerlinespacing,'REMARKS','BTLR',1,'C');
			}
			...
			if($rscount>0){
				...
				while ($obj = mysql_fetch_object($rs)) {
					...
					if($pageline>$totalrowsperpage||$rowcount>$maxlinecreated||$line>$rscount){
						...
						$pdf->Cell(50,$linehgt,'Signature over Printed Name',0,0,'C');

						$pdf->Cell($spaceinbtwn,$linehgt,'',0,0,'L');

						$pdf->Cell(30,$linehgt,'',0,0,'L');
						$pdf->Cell(50,$linehgt,'Signature over Printed Name',0,1,'C');
