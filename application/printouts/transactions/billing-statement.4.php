...
if(getNumRows($rs)==1){
	while($obj = fetch($rs)){
		...
		$rs1  = query("SELECT txn_billing_waybill.id,
							...
							trim(replace(lower(if(length(txn_waybill.consignee_city)>0,txn_waybill.consignee_city,if(length(txn_waybill.consignee_state_province)>0,txn_waybill.consignee_state_province,destinationtbl.description))),'city','')) as destination,
							...
							ifnull(
								if(mode_of_transport.id in (select id from mode_of_transport where description like '%SEA%'),
									concat(format(package_cbm,1),'cbm'),
                                    replace(
										concat(
											format(
												if(package_actual_weight<=.5,
													package_actual_weight*1000,
                                                    round(package_actual_weight,1)),if(package_actual_weight<=.5,0,1)),
											if(package_actual_weight<=.5,
												'gms',
                                                if(package_actual_weight = 1,
					...
					left join mode_of_transport on mode_of_transport.id=txn_waybill.package_mode_of_transport
						...
						order by txn_waybill.mawbl_bl,txn_waybill.waybill_number
						");
		...
		if(getNumRows($rs1)<=20) {
			while($obj1=fetch($rs1)){
				$pdf->SetFont('Arial','',9);
				$pdf->SetY($y);
				$pdf->cell(10,5,dateFormat($obj1->document_date,'m-d'),0,0,'C');
				$pdf->cell(22,5,$obj1->waybill_number,0,0);
				...
				$pdf->cell(44,5,convertWithDecimal($obj1->vatable_charges,2),0,0,'R');
				$pdf->cell(40,5,convertWithDecimal($obj1->vatable_charges,2),0,1,'R');
				$grandtotal += $obj1->vatable_charges;
				$y = $y+4;
			}
  
			$vat = convertWithDecimal($grandtotal * 0.12,2);
			$gross = convertWithDecimal($grandtotal,2);
			$net = convertWithDecimal($grandtotal + ($grandtotal * 0.12),2);
