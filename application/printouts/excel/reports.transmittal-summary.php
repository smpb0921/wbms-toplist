<?php
    ...
    while($obj=fetch($getbillininfors)){
     ...
     $qry =       "      select txn_billing.id,
                                ...
                                case
                                      when origin_destination_port.description='METRO MANILA' then txn_waybill.consignee_city
                                      else origin_destination_port.description
                                end as wbdestination,
                                services.description as wbservices,
                                txn_waybill.status as wbstatus,
                                txn_waybill.pickup_date as wbdate,
        ...
        $qry = $qry.$condition.' group by txn_billing.billing_number, txn_billing_waybill.waybill_number order by txn_waybill.pickup_date, txn_waybill.mawbl_bl, txn_waybill.waybill_number asc ';
        ...
        if($form==''||$form=='DEFAULT'){
                    ...
                    $sheet->setCellValue('A5','Attention: '.$blattn);
                     ...
                     while($obj=mysql_fetch_object($rs)){
                            ...
                            $weightuom = $seaflag==1?'cbm':($obj->wbchargeableweight>.5?'kgs':($obj->wbchargeableweight>0?'gms':''));

                            if(trim($wgt)==''){
                                $weightuom = '';
                            }
                            $billingnumber = $obj->invoice;
                    ...
                    $sheet->setCellValue('A5','Attention: '.$blattn);
                     ...
                     while($obj=mysql_fetch_object($rs)){
                            ...
                            $landflagcheck = strpos(strtoupper($mot), 'LANDFREIGHT');
                            if($landflagcheck>=0&&trim($landflagcheck)!=''){
                               $modeoftransport = 'Trucking';
                            }
                    ...
                    $sheet->setCellValue('A5','Attention: '.$blattn);
                     ...
                     $headercell = array(
                                            ...
                                            'Billing Statement No.',
                                            "Remarks",
                                            "Based on Remarks Box in System"
                     ...
                     while($obj=mysql_fetch_object($rs)){
                            ...
                            $detailcell = array(
                                                     ...
                                                     dateFormat($obj->received_date,'h:i:s A'),
