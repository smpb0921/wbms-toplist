<?php
	...
	$customqry = "
				    select published_rate.id,
						   ...
						   case
							   when published_rate.fixed_rate_flag=1 then 'YES'
							   else 'NO'
						   end as fixed_rate_flag,
						   case
							   when published_rate.rush_flag=1 then 'YES'
							   else 'NO'
						   end as rush_flag,
						   case
							   when published_rate.pull_out_flag=1 then 'YES'
							   else 'NO'
	...
	while ($obj = mysql_fetch_object($results)) {
		...
		$viewpublishedratefreightchargebtn = (userAccess(USERID,'.editpublishedratebtn')==false&&$fixedrateflag=='NO')?"<img src='../resources/img/weight_add.png' rowid='$id' title='Freight Rate based on Weight Range' class='viewpublishedratefreightchargebtn pointer' height='20px'>":'';
		...
		if(trim($viewpublishedratefreightchargebtn)!=''){
			array_push($arr, $viewpublishedratefreightchargebtn);
		}
