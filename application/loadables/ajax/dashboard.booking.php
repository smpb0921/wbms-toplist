<?php
	...
	$useronlycondition = "where txn_booking.status='LOGGED' or txn_booking.status='REJECTED'";
	...
	if(mysql_num_rows($checkuserviewconrs)>0){
		$useronlycondition = "where (txn_booking.status='LOGGED' or 
		                            txn_booking.status='REJECTED') and 
	...
	while ($obj = mysql_fetch_object($results)) {
		...
		$data['rows'][] = array(
									...
									'rowAttr'=>array(
													   ...
													   'class'=>'dash-bookingpendingrow pointer'
													)
								);
		$line++;
	}
