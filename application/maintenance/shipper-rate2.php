...
<div class="modal fade" id="addshipperratemodal">
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<div class="text-center">
					<button class='btn btn-blue2 mybtn shipperratemodal-savebtn' id='addshipperratemodal-savebtn'>Save</button>
...
<div class="modal fade" id="editshipperratemodal">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
					<div class="col-md-12">
						<form class="form-horizontal">
							...
							<input type='hidden' class='shipperratemodalid'>
							<div class='col-md-7'>
								<div class='col-md-12'>
									<div class="form-group">
										<label class='control-label'>Shipper*</label>
			...
			<div class="modal-footer">
				<div class="text-center">
					<button class='btn btn-blue2 mybtn shipperratemodal-savebtn' id='editshipperratemodal-savebtn'>Save</button>
					<button class='btn btn-blue2 mybtn modal-cancelbtn'>Cancel</button>
				</div>
			</div>
		</div>
...
<div class="modal fade" id="editshipperratefreightchargemodal">
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<div class="text-center">
					<button class='btn btn-blue2 mybtn editshipperratefreightchargemodal-savebtn' id='editshipperratemodal-savebtn'>Save</button>
					<button class='btn btn-blue2 mybtn modal-cancelbtn'>Cancel</button>
				</div>
			</div>
...
<div class="modal fade" id="uploadshipperratemodal">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <div class='page-title'>
            ...
            <div class="modal-body">
                <form class="form-horizontal" action='../scripts/shipper-rate-upload2.php' method='post' id='uploadshipperratemodal-form'  enctype='multipart/form-data' target='shipperrateuploadtransactionlogframe'>
                    <div class='col-md-4'>
                        Please make sure to follow the right format.
                        Click <a class='pointer' id='shipperrate-downloadtransactionfiletemplatebtn' href='../file-templates/shipper-rate-template2.xlsx'>here</a> to download file template.
                    </div>
                    <div class='col-md-offset-1 col-md-6'>
                    	<div class="form-group">
                    		<label class='control-label'>Shipper*</label>
                    		<select name='uploadshipperratemodal-shipper' class='form-input form-control shipperdropdownselect uploadshipperratemodal-shipper' style='width:100%'>
                    		</select>
                    	</div>
                        <div class="form-group">
                            <label class='control-label'>Select an Excel File</label>
                            <input type='file' class='form-control uploadshipperratemodal-file' name='uploadshipperratemodal-file'>
                        </div>
                    </div>
                </form>
                <br>
            ...
            <div class="modal-footer">
                <div class="text-center">
                    <button class='btn btn-blue2 mybtn' id='uploadshipperratemodal-uploadbtn'>Upload</button>
                    <button class='btn btn-blue2 mybtn modal-cancelbtn' >Cancel</button>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="shipperrate-uploadtransactionlogmodal">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <div class='page-title'>
            ...
            <div class="modal-body">
                <iframe id="shipperrateuploadtransactionlogframe" name="shipperrateuploadtransactionlogframe" height="600" width="100%" frameborder="0" scrolling="yes" style='background: #fff'></iframe>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$(tabshipperRATE+' .tagsinput').tagsinput();
		$(tabshipperRATE+' .select2').select2();


		$('.modal-dialog').draggable();
		$(tabshipperRATE+' .togglebutton').bootstrapToggle({
		      on: 'Yes',
		      off: 'No',
		      size: 'mini',
		      width: '100px'
		...
		$(tabshipperRATE+" .shipmentmodedropdownselect").select2({
	            ajax: {
		...
		$(tabshipperRATE+" .3pldropdownselect").select2({
	            ajax: {
		...
		$(tabshipperRATE+" .origindestinationdropdownselect").select2({
	            ajax: {
	    ...
	    $(tabshipperRATE+" .modeoftransportdropdownselect").select2({
	            ajax: {
	    ...
	    $(tabshipperRATE+" .servicesdropdownselect").select2({
	            ajax: {
	    ...
	    $(tabshipperRATE+" .freightcomputationdropdownselect").select2({
	            ajax: {
	    ...
	    $(tabshipperRATE+" .pouchsizedropdownselect").select2({
	            ajax: {
	   	...
	   	$(tabshipperRATE+" .zonedropdownselect").select2({
	            ajax: {
		...
		$(tabshipperRATE+" #shipperratetable").flexigrid({
				...
				buttons : [
						{name: 'Add', bclass: 'add addshipperratebtn', onpress : addshipperRate},
						{separator: true},
						{name: 'Delete', bclass: 'delete deleteshipperratebtn', onpress : deleteshipperRate},
						{separator: true},
						{name: 'Upload', bclass: 'upload uploadshipperratebtn', onpress : uploadshipperRate}
				],
				searchitems : [
						
						{display: 'Shipper', name : 'shippername', isdefault: true},
						{display: 'Type', name : 'waybill_type'},
		...
		function uploadshipperRate(){
			$('#uploadshipperratemodal').modal('show');
		
		}

		function addshipperRate(){
				$('#addshipperratemodal').modal('show');
		}

		function deleteshipperRate(){

		
			if(parseInt($('#shipperratetable .trSelected').length)>0){
				$.confirm({
					animation: 'bottom', 
					closeAnimation: 'top',
					animationSpeed: 1000,
					...
					confirm: function(){
							var data = [];
							$('#shipperratetable .trSelected').each(function(){
								data.push($(this).attr('rowid'));
							});
							$.post('../scripts/shipper-rate2.php',{deleteSelectedRows:'skj$oihdtpoa$I#@4noi4AIFNlskoRboIh4!j3sio$*yhs',data:data},function(response){

								if(response.trim()=='success'){
									$('#shipperratetable').flexOptions({
											url:'loadables/ajax/maintenance.shipper-rate2.php',
		...
		$(tabshipperRATE+" #viewshipperratefreightcharge-tbl").flexigrid({
				url: 'loadables/ajax/maintenance.shipper-rate-freight-charge2.php',
				dataType: 'json',
				colModel : [
		...
		function deleteshipperRateFreightCharge(){
			var shipperrateid = $(tabshipperRATE+' #viewshipperratefreightcharge-shipperrateid').val();
			if(parseInt($('#viewshipperratefreightcharge-tbl .viewshipperratefreightcharge-checkbox:checked').length)>0){
				$.confirm({
					...
					confirm: function(){
							...
							$.post('../scripts/shipper-rate2.php',{deleteshipperRateFreightCharge:'$jhfoFIsmdlPE#9s3#7skoRboIh4!j3sio$*yhs',data:data},function(response){
