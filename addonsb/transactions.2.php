    ...
    $qry = $conn1->prepare("SELECT group_concat(id),group_concat(concat('- ',booking_number) separator '\r\n') bookings,supervisor_mobile_number,driver_mobile_number from mod_sms_booking_notifications
        ...
        while($qry->fetch()){
            array_push($arr,array(
                ...
                "SupervisorMobile" => $SuperVisorNum,
                "DriverMobile" => $DriverNum
            ));
        }
    $qry->close();
    print_r(json_encode($arr));
...
if(isset($_POST['SMSNotsSecondStep'])){
    ...
    $arr = array();
    $qry = $conn1->prepare("SELECT group_concat(msbn.id),group_concat(concat('- ',msbn.booking_number,' - ',bkn.shipper_company_name,' - ',bkn.shipper_contact,' - ', bkn.shipper_mobile, ' - ',shipper_pickup_street_address, ' - ', 'Time Ready' ,' - ', shipment_description ) separator '\r\n') bookings,supervisor_mobile_number,driver_mobile_number from mod_sms_booking_notifications msbn
                            ...
                            and driver_reply_flag = 0 
                            and driver_rejection_sent_to_supervisor_flag = 0 
                            and bkn.status = 'APPROVED'
                            group by driver_mobile_number");

/*

SELECT group_concat(msbn.id),group_concat(concat('- ',msbn.booking_number,' - ',bkn.shipper_company_name,' - ',bkn.shipper_contact,' - ', bkn.shipper_mobile, ' - ',shipper_pickup_street_address, ' - ', 'Time Ready' ,' - ', shipment_description ) separator '\r\n') bookings,supervisor_mobile_number,driver_mobile_number from mod_sms_booking_notifications msbn
                            left join txn_booking bkn on bkn.booking_number = msbn.booking_number
    ...
    $qry = $conn1->prepare("SELECT group_concat(msbn.id),group_concat(concat('- ',msbn.booking_number) separator '\r\n') bookings,supervisor_mobile_number,driver_mobile_number,group_concat(bkn.id) bookingIds from mod_sms_booking_notifications msbn
        ...
        while($qry->fetch()){
            ...
            foreach (explode(",",$bknIds) as $key => $value) {
                query("UPDATE txn_booking set status ='APPROVED' where id = {$value}"); 
            }
        ...
        if(getNumRows($checktxnrs) > 0){
            ...
            if($booking->status == "APPROVED"){
                ...
                $qry = $conn1->prepare("UPDATE mod_sms_booking_notifications set driver_reply_flag = 1, driver_reply_timestamp = NOW() where booking_number = ?");
                    ...
                    if($qry->execute()) {
                        print_r(json_encode(
                            array(
                                "Success" => true,
                                "Message"=>"Booking with Transaction Number {$booking->booking_number} has been Acknowledged by the Driver or Helper.",
                ...
                print_r(json_encode(
                    array(
                        "Success" => false,
                        "Message"=>"Booking with Transaction Number {$booking->booking_number} is not yet approved. Unable to proceed with transaction.",
                        "Status"=>"N/A",
                        "Shippers" => $ShipperSendables,
                        "Consignees" => $ConsigneeSendables
                    )
                    ...
                    if((validateDateTime($eta)==1&&$eta!='1970-01-01 08:00:00')||$status!='TRANSFERRED') {
                        ...
                        if($currentstat == "POSTED" || $currentstat == "TRANSFERRED" || $currentstat == "LOADED" || $currentstat == "RETRIEVED" || $currentstat == "OUT FOR DELIVERY" ) {
                            ...
                            if(strtoupper($currentstat)!=strtoupper($status)) {
                                ...
                                $fetchedUser = @fetch(@query("SELECT ifnull(id,0) id from user where first_name = '{$arrsender[0]}' and last_name = '{$arrsender[0]}' and middle_name ='{$sender}'"))->id;
                ...
                if(validateDateTime($actualpickupdate)==1&&$actualpickupdate!='1970-01-01 08:00:00'){
                    ...
                    if ($currentstat != "PICKED UP") {
                        ...
                        if($currentstat == "APPROVED") {
                            ...
                            $rs = query("UPDATE txn_booking set status='PICKED UP', actual_pickup_date='$actualpickupdate', updated_by = $fetchedUser, pickup_by='$pickedupby', pickup_remarks='$remarks', last_status_update_remarks = '{$remarks}', last_status_update = now() where id='$bookingid'");
                            if($rs){

                                $historyInserted = query("INSERT into txn_booking_status_history(booking_number,status_description,date,contact,remarks,created_date,created_by)
                                   VALUES ('$bookingnumber','PICKED UP',NOW(),'$pickedupby','$remarks',NOW(),$fetchedUser)");
