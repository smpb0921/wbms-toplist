                    ...
                    if((validateDateTime($eta)==1&&$eta!='1970-01-01 08:00:00')||$status!='TRANSFERRED') {
                        ...
                        if($currentstat == "POSTED" || $currentstat == "TRANSFERRED" || $currentstat == "LOADED" || $currentstat == "RETRIEVED" || $currentstat == "OUT FOR DELIVERY" ) {
                            ...
                            if(strtoupper($currentstat)!=strtoupper($status)) {
                                                                                //, updated_by = ''
                                $rs = query("update txn_manifest set status='$status', last_status_update_remarks='$remarks', remarks = '$remarks', updated_date='$now' where id='$manifestid'"); 
                                if($rs){
                                    ...
                                    while($obj3=fetch($loopwaybillrs)) { 
                                        $waybillstathistory->insert(array('',$obj3->waybill_number,$status,$status,$remarks,$now,$userid,'NULL',$manifestnumber,'MANIFEST','NULL','NULL'));
                                                                                //, updated_by = ''
                                        query("update txn_waybill set status='$status', last_status_update_remarks='$remarks', remarks = '$remarks', updated_date = '$now' where waybill_number='$obj3->waybill_number'");
                                    }
                                    print_r(
                                        json_encode(
                                            array("Success"=>true,
    ...
    if($_POST['updateStatusPickedUp']=='kkjO#@siaah3h0$09odfj$owenxezpo92po@k@'){ 
            ...
            if(getNumRows($checktxnrs)>=1) {
                ...
                print_r(
                    json_encode(
                        array("Success"=>false,
                              "Message"=>"Booking With Transaction Number $",
                              "Status"=>"N/A",
                              "SMSReceipients"=>$sender,
                              "EmailReceipients"=>"phbhnlee@gmail.com|Bhen Ogrimen")));
