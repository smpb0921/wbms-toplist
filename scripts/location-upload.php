<?php
    ...
    if(isset($_FILES['uploadlocationmodal-file'])){
    	...
    	$file = $_FILES['uploadlocationmodal-file'];
    	$tmp = $file['tmp_name'];
    	$filename = $file['name'];
        //$ftype = $file['type'];
    	...
    	if($ftype=='.xls'||$ftype=='.csv'||$ftype=='.xlsx'){
                ...
                if($checkHeaderInfo==true){
                    ...
                    for($i=2;$i<=$lastRow;$i++){
                        ...
                        $checkifexistrs = query("select * from location 
                                                where code='$code' and
                        ...
                        if(getNumRows($checkifexistrs)>0){//EXISTING RATE - UPDATE
                            ...
                            $systemlog->logEditedInfo($prclass,$systemID,array($code,$description,$userid,$now,'NULL','NULL'),'LOCATION','Edited Location Info (UPLOAD)',$userid,$now);
                        ...
                        // Changed validation - removed $remarks!='' requirement
                            ...
                            $systemlog->logAddedInfo($prclass,array($systemID,$code,$description,$userid,$now,'NULL','NULL'),'LOCATION','New Location Added (UPLOAD)',$userid,$now);
                    ...
                    echo "Click <a class='pointer' href='../file-templates/location-template.xlsx'>here</a> to download file template<br>";
