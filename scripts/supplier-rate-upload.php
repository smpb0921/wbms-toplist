<?php
    ...
    if(isset($_FILES['uploadsupplierratemodal-file'])){
       ...
       function getPortID($code){
    	...
    	$file = $_FILES['uploadsupplierratemodal-file'];
    	$tmp = $file['tmp_name'];
    	$filename = $file['name'];
        //$ftype = $file['type'];
    	...
    	if($ftype=='.xls'||$ftype=='.csv'||$ftype=='.xlsx'){
                ...
                if($checkHeaderInfo==true){
                    ...
                    $prfcclass = new supplier_rate_freight_charge();
                    ...
                    for($i=2;$i<=$lastRow;$i++){
                        ...
                        if($tplID!=''&&($type=='DOCUMENT'||$type=='PARCEL')&&$originID!=''&&$zoneID!=''&&$pouchsizeID!=''&&(
                                    ...
                                    //if((($fromkg<=$tokg&&$fromkg>0&&$tokg>0)||($fromkg>0&&$tokg==0))&&$freightcharge>0){
                                            ...
                                            $checkifexistrs = query("select * from supplier_rate 
                                            ...
                                            if(getNumRows($checkifexistrs)>0){//EXISTING RATE
                                                ...
                                                while($obj=fetch($checkifexistrs)){
                                                 ...
                                                 array_push($txnrowexist, "<b>Details</b>: Line $i - SystemID=$systemID, 3PL=$tpl, Type=$type, Origin=$origincode, Zone=$zonecode, PouchSize=$pouchsize, FixedRateFlag=$fixedflag, FixedAmount=$fixedamount, FromKg=$fromkg, ToKg=$tokg, FreightCharge=$freightcharge, ExcessCharge=$excessfreightcharge");

                                                 $systemlog->logEditedInfo($prclass,$systemID,array('',$originID,'NULL','NULL','NULL',$fixedflagbool,0,$fixedamount,0,0,0,0,$userid,$now,'NULL','NULL',0,0,$type,$pouchsizeID,0,0,0,'NULL',$zoneID,$tplID),'supplier RATE','Edited supplier Rate Info (UPLOAD)',$userid,$now);
                                                 $prclass->update($systemID,array($originID,'NULL','NULL','NULL',$fixedflagbool,0,$fixedamount,0,0,0,0,$userid,$now,'NULL','NULL',0,0,$type,$pouchsizeID,0,0,0,'NULL',$zoneID,$tplID));


                                                 if($fixedflagbool==1){
                                                    query("delete from supplier_rate_freight_charge where supplier_rate_id='$systemID'");
                                                 }
                                                 else{
                                                    ...
                                                    $systemlog->logAddedInfo($prfcclass,array($prfreightchargerowid,$systemID,$fromkg,$tokg,$freightcharge,$excessfreightcharge,$now,$userid,'NULL','NULL'),'supplier RATE - FREIGHT CHARGE','New supplier Rate - Freight Charge (Upload)',$userid,$now);
                                                ...
                                                $userid = USERID;
                                                $now = date('Y-m-d H:i:s');
                                                $prclass->insert(array('',$originID,'NULL','NULL','NULL',$fixedflagbool,0,$fixedamount,0,0,0,0,$userid,$now,'NULL','NULL',0,0,$type,$pouchsizeID,0,0,0,'NULL',$zoneID,$tplID));
                                                $systemID = $prclass->getInsertId();
                                                $systemlog->logAddedInfo($prclass,array($systemID,$originID,'NULL','NULL','NULL',$fixedflagbool,0,$fixedamount,0,0,0,0,$userid,$now,'NULL','NULL',0,0,$type,$pouchsizeID,0,0,0,'NULL',$zoneID,$tplID),'supplier RATE','New supplier Rate Added (UPLOAD)',$userid,$now);

                                                array_push($txnwithouterror, "<b>Details</b>: Line $i - SystemID=$systemID, 3PL=$tpl, Type=$type, Origin=$origincode, Zone=$zonecode, PouchSize=$pouchsize, FixedRateFlag=$fixedflag, FixedAmount=$fixedamount, FromKg=$fromkg, ToKg=$tokg, FreightCharge=$freightcharge, ExcessCharge=$excessfreightcharge");
                                ...
                                /*if($originID!=''){
                                    ...
                                    if(getNumRows($checkifexistrs)>0){
                                        ...
                                        $systemlog->logEditedInfo($supplierrateclass,$systemID,array($systemID,$district,$city,$zipcode,$portcode,'NOCHANGE','NOCHANGE',$userid,$now,$odaflag,$odarate),'District/City/ZipCode','Edited District/City/ZipCode - Upload',$userid,$now);
                                        $supplierrateclass->update($systemID,array($district,$city,$zipcode,$portID,'NOCHANGE','NOCHANGE',$userid,$now,$odaflag,$odarate));
                                    }
                                    else{
                                        ...
                                        $supplierrateclass->insert(array('',$district,$city,$zipcode,$portID,$userid,$now,'NULL','NULL',$odaflag,$odarate));
                                        $systemID = $supplierrateclass->getInsertId();
                                        $systemlog->logAddedInfo($supplierrateclass,array($systemID,$district,$city,$zipcode,$portcode,$userid,$now,'NULL','NULL',$odaflag,$odarate),'District/City/ZipCode','New District/City/ZipCode Added - Upload',$userid,$now);
                                    }
                            ...
                            array_push($txnwithrowerror, "<b>Line</b>: $i<br><b>Error</b>: $rowtexterror <br><b>Details</b>: 3PL=$tpl, Type=$type, Origin=$origincode, Zone=$zonecode, PouchSize=$pouchsize, FixedRateFlag=$fixedflag, FixedAmount=$fixedamount, FromKg=$fromkg, ToKg=$tokg, FreightCharge=$freightcharge, ExcessCharge=$excessfreightcharge");
                    ...
                    echo "Click <a class='pointer' href='../file-templates/supplier-rate-template.xlsx'>here</a> to download file template<br>";
