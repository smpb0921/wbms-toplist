<?php
	...
	if(isset($_POST['SaveWaybillTransaction'])){
		if($_POST['SaveWaybillTransaction']=='oi$ha@3h0$0jRoihQnsRP9$nzpo92po@k@'){
			...
			$deliveryinstruction = trim($_POST['deliveryinstruction'])==''?'NULL':escapeString($_POST['deliveryinstruction']);
				...
				//if(getNumRows($checkifvalidwaybillrs)>0){
					if($maxweightallowed>=$actualweight||$maxweightallowed==0){
						...
						if(getNumRows($checkshipperrs)==1){
							while($obj=fetch($checkshipperrs)){
								if($obj->vat_flag==1){
									$zeroratedflag = 0;
								}
								else{
										...
										if(validateDate($documentdate)==1&&$documentdate!='1970-01-01'){
													...
													if($_POST['handlinginstruction']!=null){
														for($i=0;$i<count($handlinginstructionarray);$i++){
															$handlingins = array();
															if(trim($handlinginstructionarray[$i])!=''){
																array_push($handlingins, '', $waybillnumber, $handlinginstructionarray[$i]);
																//echo '-->'.$handlinginstructionarray[$i];
		...
		if($_POST['getWaybillData']=='F#@!3R3ksk#Op1NEi34smo1sonk&$'){
			...
			if(getNumRows($rs)==1){
				while($obj = fetch($rs)){
					...
					$costingrs = query("select ifnull(round(sum(amount),5),0) as amount from costing_waybill where waybill_number='$txnnumber' group by  waybill_number");
					while($obj2=fetch($costingrs)){
						$totalcost = $obj2->amount;
					}

					$grossincome = $obj->total_amount-($obj->agent_cost+$obj->freight_cost+$obj->insurance_amount+$totalcost);//($obj->agent_cost+$obj->freight_cost+$obj->insurance_amount);
					...
					$updatepaidflag = hasAccess(USERID,'#waybill-trans-updatepaidflagbtn');
					if($updatepaidflag==1||USERID==1){
						$userupdatepaidflag='true';
					}
					else{
						$userupdatepaidflag='false';
					}
					...
					$dataarray = array(
									   ...
									   "consigneeidnumber"=>utfEncode($obj->id_number),
									   ...
									   "billingstatement"=>utfEncode($obj->billingstatement),
	...
	if(isset($_POST['AddNewConsignee'])){
		if($_POST['AddNewConsignee']=='kkjO#@siaah3h0$09odfj$owenxezpo92po@k@'){
			...
			$idnumber = isset($_POST['idnumber'])&&trim($_POST['idnumber'])!=''?escapeString($_POST['idnumber']):'NULL';
			...
			if($userhasaccess==1){
				...
				if(getNumRows($checkifexistrs)>0){
					...
					$checkidnumberrs = query("select * from consignee where id_number='$idnumber'");
					...
					if(getNumRows($checkidnumberrs)>0){
						...
						$systemlog->logAddedInfo($consigneeclass,array($id,$accountnumber,$accountname,$companyname,$street,$district,$city,$province,$zipcode,$country,$userid,$now,'NULL','NULL',0,$idnumber),'CONSIGNEE','New Consignee Added (Waybill Module)',$userid,$now);
	...
	if(isset($_POST['changePaymentFlagging'])){
        if($_POST['changePaymentFlagging']=='dROi$nsFpo94dnels$4sRoi809srbmouS@1!'){
                ...
                $reference = escapeString(trim(strtoupper($_POST['paymentreference'])));
                $type = escapeString(trim(strtoupper($_POST['type'])));
                $mawbl = escapeString(trim(strtoupper($_POST['mawbl'])));
                $remarks = escapeString($_POST['remarks']);
                ...
                if($type=='VARIOUS'){
                	$checktxnrs = query("select * from txn_waybill where mawbl_bl='$mawbl'");
                }
                else{
                	$checktxnrs = query("select * from txn_waybill where waybill_number='$txnnumber'");
                }


                if(($type=='VARIOUS'&&getNumRows($checktxnrs)>0)||($type=='NON-VARIOUS'&&getNumRows($checktxnrs)==1)){
                	...
                	if(($flag==1&&trim($reference)!='')||$flag==0){
                        ...
                        if($rs){
                        	...
                        	$ref = $flag==0?'NULL':$reference;
                            $systemlog->logInfo('WAYBILL','Update Payment Status',"type: $type | waybill_number: ".$txnnumber." | mawbl: $mawbl | Flag: $flag | Reference: $reference | Remarks: $remarks",$userid,$now);
                            echo "success";
                        }
                    }
                    else{
