<?php
    ...
    if(isset($_FILES['uploadshipperratemodal-file'])&&isset($_POST['uploadshipperratemodal-shipper'])){

        $shipperid = escapeString(trim($_POST['uploadshipperratemodal-shipper']));
        $checkshipperrs = query("select * from shipper where shipper.id='$shipperid'");
        ...
        if(getNumRows($checkshipperrs)==1){
        	...
        	$file = $_FILES['uploadshipperratemodal-file'];
        	$tmp = $file['tmp_name'];
        	$filename = $file['name'];
            //$ftype = $file['type'];
        	...
        	if($ftype=='.xls'||$ftype=='.csv'||$ftype=='.xlsx'){
                    ...
                    if($checkHeaderInfo==true){
                        ...
                        $prfcclass = new shipper_rate_freight_charge();
                            ...
                            if($shipmenttypeID!=''&&$shipmentmodeID!=''&&$modeoftransportID!=''&&$tplID!=''&&($type=='DOCUMENT'||$type=='PARCEL')&&$originID!=''&&$zoneID!=''&&$pouchsizeID!=''&&(
                                        ...
                                        //if((($fromkg<=$tokg&&$fromkg>0&&$tokg>0)||($fromkg>0&&$tokg==0))&&$freightcharge>0){
                                                ...
                                                if(getNumRows($checkifexistrs)>0){//EXISTING RATE
                                                    ...
                                                    while($obj=fetch($checkifexistrs)){
                                                     ...
                                                     if($fixedflagbool==1){
                                                        query("delete from shipper_rate_freight_charge where shipper_rate_id='$systemID'");
                                                     }
                                                     else{
                                                        ...
                                                        $systemlog->logAddedInfo($prfcclass,array($prfreightchargerowid,$systemID,$fromkg,$tokg,$freightcharge,$excessfreightcharge,$now,$userid,'NULL','NULL'),'shipper RATE - FREIGHT CHARGE','New shipper Rate - Freight Charge (Upload)',$userid,$now);
                                                    ...
                                                    if($fixedflagbool==0){
                                                        $userid = USERID;
                                                        $now = date('Y-m-d H:i:s');
                                                        $x = $prfcclass->insert(array('',$systemID,$fromkg,$tokg,$freightcharge,$excessfreightcharge,$now,$userid,'NULL','NULL'));
                                    ...
                                    /*if($originID!=''){
                                        ...
                                        if(getNumRows($checkifexistrs)>0){
                                            ...
                                            $systemlog->logEditedInfo($shipperrateclass,$systemID,array($systemID,$district,$city,$zipcode,$portcode,'NOCHANGE','NOCHANGE',$userid,$now,$odaflag,$odarate),'District/City/ZipCode','Edited District/City/ZipCode - Upload',$userid,$now);
                                            $shipperrateclass->update($systemID,array($district,$city,$zipcode,$portID,'NOCHANGE','NOCHANGE',$userid,$now,$odaflag,$odarate));
                                        }
                                        else{
                                            ...
                                            $shipperrateclass->insert(array('',$district,$city,$zipcode,$portID,$userid,$now,'NULL','NULL',$odaflag,$odarate));
                                            $systemID = $shipperrateclass->getInsertId();
                                            $systemlog->logAddedInfo($shipperrateclass,array($systemID,$district,$city,$zipcode,$portcode,$userid,$now,'NULL','NULL',$odaflag,$odarate),'District/City/ZipCode','New District/City/ZipCode Added - Upload',$userid,$now);
                                        }
                        ...
                        echo "Click <a class='pointer' href='../file-templates/shipper-rate-template.xlsx'>here</a> to download file template<br>";
            ...
            echo "Invalid Shipper. Please re-select.";
        }
