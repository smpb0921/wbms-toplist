				...
				if(getNumRows($checkshipperrs)==1){
						...
						//if(validateDate($pickupdate)==1&&$pickupdate!='1970-01-01'&&$pickupdatenum>=$currentdatenum){
							...
							if($source=='add'){
								...
								/*$systemlog->logEditedInfo($pwyh,$id,array('',$pwynumber,$status,$rcvnum,$remarks,'NULL','NULL',$now,$userid,'NOCHANGE'),'Edited Putaway Transaction',$userid,$now);/// log should be before update is made
								$pwyh->update($id,array($pwynumber,'NOCHANGE',$rcvnum,$remarks,'NOCHANGE','NOCHANGE',$now,$userid,'NOCHANGE'));
								*/
							}
