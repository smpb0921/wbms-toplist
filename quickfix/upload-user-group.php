				...
				$now = date('Y-m-d H:i:s');
					...
					for($i=1;$i<$numberofrows;$i++){
						...
						if($code!='NULL'&&$desc!='NULL'){
							...
							$checkifexistrs = mysql_query("select * from user_group where code='$code'");

							if(mysql_num_rows($checkifexistrs)>0){
							}
							else{
								mysql_query("insert into user_group(code,description,created_by,created_date) values('$code','$desc',1,'$now')");
