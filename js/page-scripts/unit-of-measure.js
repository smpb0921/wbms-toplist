$(document).off('click','.editunitofmeasurebtn').on('click','.editunitofmeasurebtn',function(){
	var modal = "#editunitofmeasuremodal";
	$(modal+' .code').val($(this).attr('code'));
	$(modal+' .description').val($(this).attr('desc'));
	$(modal+' .unitofmeasuremodalid').val($(this).attr('rowid'));
});

/***************************** SAVE | EDIT **************************************************/
$(document).off('click','.unitofmeasuremodal-savebtn:not(".disabled")').on('click','.unitofmeasuremodal-savebtn:not(".disabled")',function(){
	...
	if(modal=='#editunitofmeasuremodal'){
		id = $(modal+' .unitofmeasuremodalid').val();
		source = 'edit';
	...
	if(code==''){
		...
		$.post(server+'unit-of-measure.php',{unitofmeasureSaveEdit:'j$isnDPo#P3sll3p23a3!@3kzlsO!mslo#k@',source:source,id:id,code:code,desc:desc},function(data){
