$(document).off('click','.editshipmenttypebtn').on('click','.editshipmenttypebtn',function(){
	var modal = "#editshipmenttypemodal";
	$(modal+' .code').val($(this).attr('code'));
	$(modal+' .description').val($(this).attr('desc'));
	$(modal+' .shipmenttypemodalid').val($(this).attr('rowid'));
});

/***************************** SAVE | EDIT **************************************************/
$(document).off('click','.shipmenttypemodal-savebtn:not(".disabled")').on('click','.shipmenttypemodal-savebtn:not(".disabled")',function(){
	...
	if(modal=='#editshipmenttypemodal'){
		id = $(modal+' .shipmenttypemodalid').val();
		source = 'edit';
	...
	if(code==''){
		...
		$.post(server+'shipment-type.php',{shipmenttypeSaveEdit:'j$isnDPo#P3sll3p23a3!@3kzlsO!mslo#k@',source:source,id:id,code:code,desc:desc},function(data){
