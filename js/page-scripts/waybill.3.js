...
function disableFieldsWB(){
	$(contentWB+' .errordiv').empty();
	$(inputfieldsWB+' input:not(".alwaysdisabled"),'+inputfieldsWB+' textarea:not(".alwaysdisabled"),'+inputfieldsWB+' select:not([aria-controls="waybill-otherchargestbl"])').attr('disabled',true);
	...
	$(contentWB+' .searchbtn').removeClass('disabled').addClass('active').removeClass('hidden');
	...
	$(contentWB+' .waybill-consignee-secondary').addClass('alwaysdisabled').attr('disabled','disabled');
...
function clearAllWB(){
	...
	$(inputfieldsWB+' textarea.form-control').text('').val('');
	$(inputfieldsWB+' select:not([aria-controls="waybill-otherchargestbl"])').val('').trigger('change');
	$(inputfieldsWB+' .header-errordiv').empty();
	$(inputfieldsWB+' .detail-errordiv').empty();
	$(contentWB+' .statusdiv').html('<br>');
	$(contentWB+' #waybill-otherchargestbl').DataTable().clear().search('').draw();
	$(contentWB+' #waybill-packagedimensionsmodaltbl').DataTable().clear().search('').draw();

	$(contentWB+' .waybill-consignee-secondary').val('');
	
	$(contentWB+' .waybill-rushflag, '+contentWB+' .waybill-pulloutflag, '+contentWB+' .waybill-odaflag').val('0').trigger('change');
	$(contentWB+' .waybill-waybilltype').val('PARCEL').trigger('change');
}

function clearNewPickupAddressFieldsWB(){
	var modal = contentWB+" #waybill-shipperpickupaddresslookup";
	$(modal+' .inputtxtfld').val('');
	$(modal+' .inputslctfld').empty().trigger('change');
}
...
function computeTotalsEWC(){
		...
		var vatvalue = totalvatableamount*.12;
		...
		$(contentWB+' .editwaybillchargesmodal-regularcharges').val(totalregularcharges).number( true, decimalplaces );
		$(contentWB+' .editwaybillchargesmodal-otherchargesvatable').val(totalotherchargesvatable).number( true, decimalplaces );
		$(contentWB+' .editwaybillchargesmodal-otherchargesnonvatable').val(totalotherchargesnonvatable).number( true, decimalplaces );
		$(contentWB+' .editwaybillchargesmodal-subtotal').val(totalvatableamount).number( true, decimalplaces );
		
		var zeroratedflag = $(contentWB+' .editwaybillchargesmodal-zeroratedcheckbox').prop('checked');
		if(zeroratedflag==true){
			$(contentWB+' .editwaybillchargesmodal-totalamount').val(totalamountnovat).number( true, decimalplaces );
			$(contentWB+' .editwaybillchargesmodal-vat').val(0).number( true, decimalplaces );

			
		}
		else if(zeroratedflag==false){
			$(contentWB+' .editwaybillchargesmodal-totalamount').val(totalamountplusvat).number( true, decimalplaces );
			$(contentWB+' .editwaybillchargesmodal-vat').val(vatvalue).number( true, decimalplaces );
...
function computeRatesWB(){
	if(processWB=='add'||processWB=='edit'){
		...
		$.post(server+'waybill.php',{waybillComputation:'sdf#io2s9$dlIP$psLn!#oid($)soep$8%syo7',tpl:tpl,origin:origin,destination:destination,modeoftransport:modeoftransport,services:services,cbm:vwcbm,actualweight:actualweight,declaredvalue:declaredvalue,numberofpackage:numberofpackage,vw:vw,waybilltype:waybilltype,rushflag:rushflag,pulloutflag:pulloutflag,shipperid:shipperid,pouchsize:pouchsize,handlinginstruction:handlinginstruction,consigneeid:consigneeid,amountforcollection:amountforcollection,odaflag:odaflag,expresstransactiontype:expresstransactiontype,parceltype:parceltype},function(data){
			//alert(data);
			data = $.parseJSON(data);
			var decimalplaces = data["decimalplaces"];
...
/************* NEW BTN *********************/
$(document).off('click',contentWB+' #waybill-trans-newbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-newbtn:not(".disabled")',function(){
	processWB = 'add';
	enableFieldsWB();
	clearAllWB();
	...
	$(contentWB+' #pgtxnwaybill-id').val('').removeAttr('pwynumber');
	$(contentWB+' .statusdiv').html('RECEIVED');	
			...
			$.post("../config/post-functions.php",{defaulDoortoDoor:'oiskus49Fnla3#Oih4noiI$IO@Y#*h@o3sk'},function(data){
				...
				if(data['response']=='success'){
					if(data["service"].trim()!=null){
						$(contentWB+" .waybill-services").empty().append('<option selected value="'+data["serviceid"]+'">'+data["service"]+'</option>').trigger('change');
					}
					else{
						$(contentWB+" .waybill-services").empty().trigger('change');
					}
				}
				$(contentWB+' .waybill-origin').empty().append(`<option value='106'>METRO MANILA</option>`).val(106).trigger('change');
				$(contentWB+' .waybill-destinationroute').empty().append(`<option value='9'>NA</option>`).val(9).trigger('change');
				$(contentWB+' .waybill-deliveryinstruction').empty().append(`<option value='3'>RUSH!! PLEASE RETURN POD</option>`).val(3).trigger('change');
				$(contentWB+' .waybill-modeoftransport').empty().append(`<option value='1'>Airfreight Domestic</option>`).val(1).trigger('change');
				


				$(contentWB+" .waybill-3pl").empty().append('<option selected value=1>CBL</option>').trigger('change');
				$(contentWB+' .transactionnumber').focus().select();
			});
...
/************* EDIT BTN *********************/
$(document).off('click',contentWB+' #waybill-trans-editbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-editbtn:not(".disabled")',function(){
...
/************** CANCEL BTN *******************/
$(document).off('click',contentWB+' #waybill-trans-cancelbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-cancelbtn:not(".disabled")',function(){
	processWB = '';
	...
	if(currentWaybillTxn!=''){
		...
		$(contentWB+' .transactionnumber').val('');
		$(contentWB+' .topbuttonsdiv').html("<div class='padded-with-border-engraved topbuttonswrapper'><div class='button-group'><div class='button-group-btn active' title='New' id='waybill-trans-newbtn'><img src='../resources/img/add.png'></div></div></div>");//<div class='button-group-btn active' title='Upload' id='waybill-trans-uploadbtn'><img src='../resources/img/upload.png'></div>
	}

	setTimeout(function(){$('#loading-img').addClass('hidden');},500);
	$('.content').animate({scrollTop:0},300);
});
/************ CANCEL BTN - END *****************/

/************** SAVE BTN *******************/
$(document).off('click',contentWB+' #waybill-trans-savebtn:not(".disabled")').on('click',contentWB+' #waybill-trans-savebtn:not(".disabled")',function(){
	...
	/*if(waybillnumber.trim()==''){
		$(contentWB+' .header-errordiv').html("<div class='message'><div class='message-content'><span class='closemessage'>&times;</span>Please provide waybill number</div></div>");
	...
	/*else if(waybilltype=='PARCEL'&&(costcenter.trim()==''&&paymode.toUpperCase()=='SERVICE CARGO')){
		...
		//button.removeClass('disabled');
		$(contentWB+' .header-errordiv, '+contentWB+' .detail-errordiv').empty();
		$.post(server+'waybill.php',{SaveWaybillTransaction:'oi$ha@3h0$0jRoihQnsRP9$nzpo92po@k@',agent:agent,id:id,mawbl:mawbl,reference:reference,waybillnumber:waybillnumber,bookingnumber:bookingnumber,origin:origin,destination:destination,destinationroute:destinationroute,onholdflag:onholdflag,onholdremarks:onholdremarks,pickupdate:pickupdate,remarks:remarks,documentdate:documentdate,shipperid:shipperid,shipperaccountnumber:shipperaccountnumber,shipperaccountname:shipperaccountname,shippertel:shippertel,shippercompanyname:shippercompanyname,shipperstreet:shipperstreet,shipperdistrict:shipperdistrict,shippercity:shippercity,shipperprovince:shipperprovince,shipperzipcode:shipperzipcode,shippercountry:shippercountry,pickupstreet:pickupstreet,pickupdistrict:pickupdistrict,pickupcity:pickupcity,pickupprovince:pickupprovince,pickupzipcode:pickupzipcode,pickupcountry:pickupcountry,consigneeid:consigneeid,consigneeaccountnumber:consigneeaccountnumber,consigneeaccountname:consigneeaccountname,consigneetel:consigneetel,consigneecompanyname:consigneecompanyname,consigneestreet:consigneestreet,consigneedistrict:consigneedistrict,consigneecity:consigneecity,consigneeprovince:consigneeprovince,consigneezipcode:consigneezipcode,consigneecountry:consigneecountry,numberofpackage:numberofpackage,declaredvalue:declaredvalue,actualweight:actualweight,vwcbm:vwcbm,chargeableweight:chargeableweight,freight:freight,valuation:valuation,totalamount:totalamount,vat:vat,zeroratedflag:zeroratedflag,services:services,modeoftransport:modeoftransport,deliveryinstruction:deliveryinstruction,accompanyingdocument:accompanyingdocument,transportcharges:transportcharges,paymode:paymode,carrier:carrier,shipperrepname:shipperrepname,shipmentdescription:shipmentdescription,source:source,lineamount:lineamount,linedesc:linedesc,freightcomputation:freightcomputation,insurancerate:insurancerate,fuelrate:fuelrate,bunkerrate:bunkerrate,minimumrate:minimumrate,subtotal:subtotal,linepackagedimlength:linepackagedimlength,linepackagedimwidth:linepackagedimwidth,linepackagedimheight:linepackagedimheight,linepackagedimqty:linepackagedimqty,linepackagedimvw:linepackagedimvw,linepackagedimcbm:linepackagedimcbm,deliverydate:deliverydate,handlinginstruction:handlinginstruction,linepackagedimuom:linepackagedimuom,linepackagedimactualweight:linepackagedimactualweight,vw:vw,costcenter:costcenter,shipperpodinstruction:shipperpodinstruction,amountforcollection:amountforcollection,waybillstatus:waybillstatus,waybilltype:waybilltype,rushflag:rushflag,pulloutflag:pulloutflag,pouchsize:pouchsize,baseoda:baseoda,shipperoda:shipperoda,oda:oda,pulloutfee:pulloutfee,fixedrateamount:fixedrateamount,totalhandlingcharges:totalhandlingcharges,returndocumentfee:returndocumentfee,waybillfee:waybillfee,securityfee:securityfee,docstampfee:docstampfee,totalregularcharges:totalregularcharges,totalotherchargesvatable:totalotherchargesvatable,totalotherchargesnonvatable:totalotherchargesnonvatable,linevatflag:linevatflag,expresstransactiontype:expresstransactiontype,odaflag:odaflag,parceltype:parceltype,tpl:tpl,secondaryrecipient:secondaryrecipient,shippercontact:shippercontact,brand:brand,costcentercode:costcentercode,costcenter:costcenter,buyercode:buyercode,contractnumber:contractnumber,customernumber:customernumber,project:project,parkingslot:parkingslot,blockunitdistrict:blockunitdistrict,lotfloor:lotfloor,freightcost:freightcost},function(data){
			...
			if(data['response']=='success'){
				...
				say("Unable to save changes. Actual Weight exceeds max weight allowed for selected pouch size. <br>Max Weight: "+data['pouchsizemaxweight']+"<br>Actual Weight: "+actualweight);
				$(contentWB+' .waybill-actualweight').focus();
				$('#loading-img').addClass('hidden');
				button.removeClass('disabled');
...
/************************* VOID BTN ***********************/
$(document).off('click',contentWB+' #waybill-trans-voidbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-voidbtn:not(".disabled")',function(){
...
/************* POST BTN *********************/
$(document).off('click',contentWB+' #waybill-trans-postbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-postbtn:not(".disabled")',function(){
...
$(document).off('click',contentWB+' .othercharges-insertbtn').on('click',contentWB+' .othercharges-insertbtn',function(){
...
$(document).off('click',contentWB+' .othercharges-removebtn').on('click',contentWB+' .othercharges-removebtn',function(){
...
function waybillOtherChargesClearFields(){
	    $(contentWB+' .othercharges-amountfld').val('').focus();
	    $(contentWB+' .othercharges-descriptionfld').empty().trigger('change');
}

$(document).off('click',contentWB+' .othercharges-clearbtn ').on('click',contentWB+' .othercharges-clearbtn',function(){
		waybillOtherChargesClearFields();
});
////// CLEAR RATE INPUT FIELDS END

/****************** OTHER CHARGES TABLE - END *******************/
$(document).off('change',contentWB+' .waybill-handlinginstruction').on('change',contentWB+' .waybill-handlinginstruction',function(){
	//computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-rushflag').on('change',contentWB+' .waybill-rushflag',function(){
	//computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-pulloutflag').on('change',contentWB+' .waybill-pulloutflag',function(){
	//computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-odaflag').on('change',contentWB+' .waybill-odaflag',function(){
	//computeRatesWB();
});
$(document).off('keyup',contentWB+' .waybill-numberofpackages').on('keyup',contentWB+' .waybill-numberofpackages',function(){
	//computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-waybilltype').on('change',contentWB+' .waybill-waybilltype',function(){
	var type = $(this).val();
...
$(document).off('change',contentWB+' .waybill-parceltype').on('change',contentWB+' .waybill-parceltype',function(){
	computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-pouchsize').on('change',contentWB+' .waybill-pouchsize',function(){
	computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-origin').on('change',contentWB+' .waybill-origin',function(){
	computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-destination').on('change',contentWB+' .waybill-destination',function(){
	computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-expresstransactiontype').on('change',contentWB+' .waybill-expresstransactiontype',function(){
	//computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-modeoftransport').on('change',contentWB+' .waybill-modeoftransport',function(){
	//computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-services').on('change',contentWB+' .waybill-services',function(){
	//computeRatesWB();
});
$(document).off('change',contentWB+' .waybill-zeroratedcheckbox').on('change',contentWB+' .waybill-zeroratedcheckbox',function(){
	computeRatesWB();
});
$(document).off('focusout',contentWB+' .waybill-amountforcollection').on('focusout',contentWB+' .waybill-amountforcollection',function(){
	//computeRatesWB();
});
$(document).off('focusout',contentWB+' .waybill-declaredvalue').on('focusout',contentWB+' .waybill-declaredvalue',function(){
	//computeRatesWB();
});
$(document).off('focusout',contentWB+' .waybill-actualweight').on('focusout',contentWB+' .waybill-actualweight',function(){
	computeRatesWB();
});
$(document).off('focusout',contentWB+' .waybill-vw').on('focusout',contentWB+' .waybill-vw',function(){
	computeRatesWB();
});
$(document).off('focusout',contentWB+' .waybill-vwcbm').on('focusout',contentWB+' .waybill-vwcbm',function(){
	computeRatesWB();
});

$(document).off('change',contentWB+' .waybill-3pl').on('change',contentWB+' .waybill-3pl',function(){
	computeRatesWB();
});


$(document).off('change',contentWB+' .editwaybillchargesmodal-zeroratedcheckbox').on('change',contentWB+' .editwaybillchargesmodal-zeroratedcheckbox',function(){
	computeTotalsEWC();
});
...
$(document).off('change',contentWB+' .waybill-bookingnumber').on('change',contentWB+' .waybill-bookingnumber',function(){
	...
	if((processWB=='add'||processWB=='edit')&&bookingnumber!=''&&bookingnumber!=null){
		$.post(server+'booking.php',{getBookingData:'F#@!3R3ksk#Op1NEi34smo1sonk&$',txnnumber:bookingnumber},function(response){
			if(response.trim()!='INVALID'){
				...
				if(data["origin"]!=null){
					$(inputfieldsWB+" .waybill-origin").empty().append('<option selected value="'+data["originid"]+'">'+data["origin"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-origin").empty().trigger('change');
				}

				if(data["destination"]!=null){
					$(inputfieldsWB+" .waybill-destination").empty().append('<option selected value="'+data["destinationid"]+'">'+data["destination"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-destination").empty().trigger('change');
				}

				if(data["shipperdistrict"]!=null){
					$(inputfieldsWB+" .waybill-shipper-district").empty().append('<option selected value="'+data["shipperdistrict"]+'">'+data["shipperdistrict"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-district").empty().trigger('change');
				}
				if(data["shippercity"]!=null){
					$(inputfieldsWB+" .waybill-shipper-city").empty().append('<option selected value="'+data["shippercity"]+'">'+data["shippercity"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-city").empty().trigger('change');
				}
				if(data["shipperprovince"]!=null){
					$(inputfieldsWB+" .waybill-shipper-province").empty().append('<option selected value="'+data["shipperprovince"]+'">'+data["shipperprovince"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-province").empty().trigger('change');
				}
				if(data["shipperzipcode"]!=null){
					$(inputfieldsWB+" .waybill-shipper-zipcode").empty().append('<option selected value="'+data["shipperzipcode"]+'">'+data["shipperzipcode"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-zipcode").empty().trigger('change');
				}


				if(data["shippercountry"]!=null){
					$(inputfieldsWB+" .waybill-shipper-country").empty().append('<option selected value="'+data["shippercountry"]+'">'+data["shippercountry"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-country").empty().trigger('change');
				}

				if(data["pickupdistrict"]!=null){
					$(inputfieldsWB+" .waybill-shipper-pickupdistrict").empty().append('<option selected value="'+data["pickupdistrict"]+'">'+data["pickupdistrict"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-pickupdistrict").empty().trigger('change');
				}
				if(data["pickupcity"]!=null){
					$(inputfieldsWB+" .waybill-shipper-pickupcity").empty().append('<option selected value="'+data["pickupcity"]+'">'+data["pickupcity"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-pickupcity").empty().trigger('change');
				}
				if(data["pickupprovince"]!=null){
					$(inputfieldsWB+" .waybill-shipper-pickupprovince").empty().append('<option selected value="'+data["pickupprovince"]+'">'+data["pickupprovince"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-pickupprovince").empty().trigger('change');
				}
				if(data["pickupzipcode"]!=null){
					$(inputfieldsWB+" .waybill-shipper-pickupzipcode").empty().append('<option selected value="'+data["pickupzipcode"]+'">'+data["pickupzipcode"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-pickupzipcode").empty().trigger('change');
				}

				if(data["pickupcountry"]!=null){
					$(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().append('<option selected value="'+data["pickupcountry"]+'">'+data["pickupcountry"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().trigger('change');
				}

				if(data["consigneedistrict"]!=null){
					$(inputfieldsWB+" .waybill-consignee-district").empty().append('<option selected value="'+data["consigneedistrict"]+'">'+data["consigneedistrict"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-consignee-district").empty().trigger('change');
				}
				if(data["consigneecity"]!=null){
					$(inputfieldsWB+" .waybill-consignee-city").empty().append('<option selected value="'+data["consigneecity"]+'">'+data["consigneecity"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-consignee-city").empty().trigger('change');
				}
				if(data["consigneeprovince"]!=null){
					$(inputfieldsWB+" .waybill-consignee-province").empty().append('<option selected value="'+data["consigneeprovince"]+'">'+data["consigneeprovince"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-consignee-province").empty().trigger('change');
				}
				if(data["consigneezipcode"]!=null){
					$(inputfieldsWB+" .waybill-consignee-zipcode").empty().append('<option selected value="'+data["consigneezipcode"]+'">'+data["consigneezipcode"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-consignee-zipcode").empty().trigger('change');
				}


				if(data["consigneecountry"]!=null){
					$(inputfieldsWB+" .waybill-consignee-country").empty().append('<option selected value="'+data["consigneecountry"]+'">'+data["consigneecountry"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-consignee-country").empty().trigger('change');
				}

				if(data["service"]!=null){
					$(inputfieldsWB+" .waybill-services").empty().append('<option selected value="'+data["serviceid"]+'">'+data["service"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-services").empty().trigger('change');
				}
				if(data["modeoftransport"]!=null){
					$(inputfieldsWB+" .waybill-modeoftransport").empty().append('<option selected value="'+data["modeoftransportid"]+'">'+data["modeoftransport"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-modeoftransport").empty().trigger('change');
				}
				if(data["handlinginstruction"]!=null){
					$(inputfieldsWB+" .waybill-handlinginstruction").empty().append('<option selected value="'+data["handlinginstructionid"]+'">'+data["handlinginstruction"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-handlinginstruction").empty().trigger('change');
				}


				if(data["paymode"]!=null){
					$(inputfieldsWB+" .waybill-paymode").empty().append('<option selected value="'+data["paymode"]+'">'+data["paymode"]+'</option>').trigger('change');
				}
				else{
					$(inputfieldsWB+" .waybill-paymode").empty().trigger('change');
				}
...
/*************** INPUT GROUP BUTTONS ****************************/
$(document).off('click',contentWB+' .inputgroupicon:not(".disabled")').on('click',contentWB+' .inputgroupicon:not(".disabled")',function(){
	if(processWB=='add'||processWB=='edit'){
...
$(document).off('click',contentWB+' .inputgroupicon[alwaysshow]').on('click',contentWB+' .inputgroupicon[alwaysshow]',function(){
	if(processWB==''){
		var modal = $(this).data('modal');
		$(modal).modal('show');
	}
...
$(document).off('click',contentWB+' #waybill-pickupaddresslookupbtn:not(".disabled")').on('click',contentWB+' #waybill-pickupaddresslookupbtn:not(".disabled")',function(){
	var modal = $(this).data('modal');
...
/********************* TABLEROW EVENTS ********************************/
$(document).off('dblclick',contentWB+' .shipperlookuprow:not(".disabled")').on('dblclick',contentWB+' .shipperlookuprow:not(".disabled")',function(){
		...
		$.post(server+'shipper.php',{ShipperGetInfo:'kjoI$H2oiaah3h0$09jDppo92po@k@',id:id},function(data){
			...
			if(rsp['response']=='success'){
				...
				if(rsp["companydistrict"]!=null){
			        $(inputfieldsWB+" .waybill-shipper-district").empty().append('<option selected value="'+rsp["companydistrict"]+'">'+rsp["companydistrict"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-shipper-district").empty().trigger('change');
			    }
			    if(rsp["companycity"]!=null){
			        $(inputfieldsWB+" .waybill-shipper-city").empty().append('<option selected value="'+rsp["companycity"]+'">'+rsp["companycity"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-shipper-city").empty().trigger('change');
			    }
			    if(rsp["companyzipcode"]!=null){
			        $(inputfieldsWB+" .waybill-shipper-zipcode").empty().append('<option selected value="'+rsp["companyzipcode"]+'">'+rsp["companyzipcode"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-shipper-zipcode").empty().trigger('change');
			    }
			    if(rsp["companyprovince"]!=null){
			        $(inputfieldsWB+" .waybill-shipper-province").empty().append('<option selected value="'+rsp["companyprovince"]+'">'+rsp["companyprovince"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-shipper-province").empty().trigger('change');
			    }
				if(rsp["companycountry"]!=null){
			        $(inputfieldsWB+" .waybill-shipper-country").empty().append('<option selected value="'+rsp["companycountry"]+'">'+rsp["companycountry"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-shipper-country").empty().trigger('change');
			    }
			    ...
			    if(rsp["paymode"]!=null){
			        $(inputfieldsWB+" .waybill-paymode").empty().append('<option selected value="'+rsp["paymode"]+'">'+rsp["paymode"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-paymode").empty().trigger('change');
			    }
			    	...
			    	$.post(server+'booking.php',{ShipperDefaultPickupAddressGetInfo:'ooi3h$9apsojespriaoi#@po92po@k@',id:id},function(data2){
			    		...
			    		if(rsp2['response']=='success'){
							...
							if(rsp2["district"]!=null){
						        $(inputfieldsWB+" .waybill-shipper-pickupdistrict").empty().append('<option selected value="'+rsp2["district"]+'">'+rsp2["district"]+'</option>').trigger('change');
						    }
						    else{
						        $(inputfieldsWB+" .waybill-shipper-pickupdistrict").empty().trigger('change');
						    }
						    if(rsp2["city"]!=null){
						        $(inputfieldsWB+" .waybill-shipper-pickupcity").empty().append('<option selected value="'+rsp2["city"]+'">'+rsp2["city"]+'</option>').trigger('change');
						    }
						    else{
						        $(inputfieldsWB+" .waybill-shipper-pickupcity").empty().trigger('change');
						    }
						    if(rsp2["province"]!=null){
						        $(inputfieldsWB+" .waybill-shipper-pickupprovince").empty().append('<option selected value="'+rsp2["province"]+'">'+rsp2["province"]+'</option>').trigger('change');
						    }
						    else{
						        $(inputfieldsWB+" .waybill-shipper-pickupprovince").empty().trigger('change');
						    }
						    if(rsp2["zipcode"]!=null){
						        $(inputfieldsWB+" .waybill-shipper-pickupzipcode").empty().append('<option selected value="'+rsp2["zipcode"]+'">'+rsp2["zipcode"]+'</option>').trigger('change');
						    }
						    else{
						        $(inputfieldsWB+" .waybill-shipper-pickupzipcode").empty().trigger('change');
						    }
							if(rsp2["country"]!=null){
						        $(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().append('<option selected value="'+rsp2["country"]+'">'+rsp2["country"]+'</option>').trigger('change');
						    }
						    else{
						        $(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().trigger('change');
						    }
				    		...
				    		$(contentWB+' .waybill-shipper-contactperson').focus().select();
...
$(document).off('dblclick',contentWB+' .shipperpickupaddresslookuprow:not(".disabled")').on('dblclick',contentWB+' .shipperpickupaddresslookuprow:not(".disabled")',function(){
	...
	$(document).on('hidden.bs.modal',contentWB+' '+modal,function(){
		...
		if(row.find("td[abbr='pickup_district'] div").text().trim()!=null&&row.find("td[abbr='pickup_district'] div").text().trim()!=''){
			$(inputfieldsWB+" .waybill-shipper-pickupdistrict").empty().append('<option selected value="'+row.find("td[abbr='pickup_district'] div").text()+'">'+row.find("td[abbr='pickup_district'] div").text()+'</option>').trigger('change');
		}
		else{
		    $(inputfieldsWB+" .waybill-shipper-pickupdistrict").empty().trigger('change');
		}
		if(row.find("td[abbr='pickup_city'] div").text().trim()!=null&&row.find("td[abbr='pickup_city'] div").text().trim()!=''){
			$(inputfieldsWB+" .waybill-shipper-pickupcity").empty().append('<option selected value="'+row.find("td[abbr='pickup_city'] div").text()+'">'+row.find("td[abbr='pickup_city'] div").text()+'</option>').trigger('change');
		}
		else{
		    $(inputfieldsWB+" .waybill-shipper-pickupcity").empty().trigger('change');
		}
		if(row.find("td[abbr='pickup_zip_code'] div").text().trim()!=null&&row.find("td[abbr='pickup_zip_code'] div").text().trim()!=''){
			$(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().append('<option selected value="'+row.find("td[abbr='pickup_zip_code'] div").text()+'">'+row.find("td[abbr='pickup_zip_code'] div").text()+'</option>').trigger('change');
		}
		else{
		    $(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().trigger('change');
		}
		if(row.find("td[abbr='pickup_state_province'] div").text().trim()!=null&&row.find("td[abbr='pickup_state_province'] div").text().trim()!=''){
			$(inputfieldsWB+" .waybill-shipper-pickupprovince").empty().append('<option selected value="'+row.find("td[abbr='pickup_state_province'] div").text()+'">'+row.find("td[abbr='pickup_state_province'] div").text()+'</option>').trigger('change');
		}
		else{
		    $(inputfieldsWB+" .waybill-shipper-pickupprovince").empty().trigger('change');
		}
		if(row.find("td[abbr='pickup_country'] div").text().trim()!=null&&row.find("td[abbr='pickup_country'] div").text().trim()!=''){
			$(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().append('<option selected value="'+row.find("td[abbr='pickup_country'] div").text()+'">'+row.find("td[abbr='pickup_country'] div").text()+'</option>').trigger('change');
		}
		else{
		    $(inputfieldsWB+" .waybill-shipper-pickupcountry").empty().trigger('change');
		}
		...
		$.post(server+'consignee.php',{ConsigneeGetInfo:'kjoI$H2oiaah3h0$09jDppo92po@k@',id:id},function(data){
			...
			if(rsp['response']=='success'){
				...
				if(rsp["district"]!=null){
			        $(inputfieldsWB+" .waybill-consignee-district").empty().append('<option selected value="'+rsp["district"]+'">'+rsp["district"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-consignee-district").empty().trigger('change');
			    }
			    if(rsp["city"]!=null){
			        $(inputfieldsWB+" .waybill-consignee-city").empty().append('<option selected value="'+rsp["city"]+'">'+rsp["city"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-consignee-city").empty().trigger('change');
			    }
			    if(rsp["zipcode"]!=null){
			        $(inputfieldsWB+" .waybill-consignee-zipcode").empty().append('<option selected value="'+rsp["zipcode"]+'">'+rsp["zipcode"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-consignee-zipcode").empty().trigger('change');
			    }
			    ...
			    if(rsp["province"]!=null){
			        $(inputfieldsWB+" .waybill-consignee-province").empty().append('<option selected value="'+rsp["province"]+'">'+rsp["province"]+'</option>').trigger('change');
			    ...
			    if(rsp["provinceid"]!=''&&rsp["provinceid"]!=null){
			    	$(inputfieldsWB+" .waybill-destination").empty().append('<option selected value="'+rsp["provinceid"]+'">'+rsp["province"]+'</option>').trigger('change');
			    }
			    else{
			    	$(inputfieldsWB+" .waybill-destination").empty().trigger('change');
			    }
				...
				if(rsp["country"]!=null){
			        $(inputfieldsWB+" .waybill-consignee-country").empty().append('<option selected value="'+rsp["country"]+'">'+rsp["country"]+'</option>').trigger('change');
			    }
			    else{
			        $(inputfieldsWB+" .waybill-consignee-country").empty().trigger('change');
			    }
			    ...
			    $.post(server+'booking.php',{ConsigneeDefaultContactGetInfo:'oj94oifof#o@odlspriaoi#@po92po@k@',id:id},function(data1){
			    	...
			    	$(contentWB+' .waybill-consignee-telephone').focus().select();
...
$(document).off('dblclick',contentWB+' .consigneelookuprow:not(".disabled")').on('dblclick',contentWB+' .consigneelookuprow:not(".disabled")',function(){
	var id = $(this).attr('rowid');
	var modal = '#'+$(this).closest('.modal').attr('id');
	var row = $(this);
	row.addClass('disabled');
		...
		$.post(server+'consignee.php',{ConsigneeGetInfo:'kjoI$H2oiaah3h0$09jDppo92po@k@',id:id},function(data){
			...
			if(rsp['response']=='success'){
				...
				if(rsp['odaflag']==1||rsp['odaflag']==0){
					$(inputfieldsWB+' .waybill-odaflag').attr('disabled','').val(rsp['odaflag']).trigger('change');
				}
				else{
					$(inputfieldsWB+' .waybill-odaflag').removeAttr('disabled').val(0).trigger('change');
				}
...
$(document).off('show.bs.modal',contentWB+' #waybill-shipperpickupaddresslookup').on('show.bs.modal',contentWB+' #waybill-shipperpickupaddresslookup',function(){

	var modal = '#'+$(this).attr('id');
	var selectedshipperID = $(modal+' .waybill-shipperpickupaddresslookup-shipperid').val(); 
	$(contentWB+" #waybill-shipperpickupaddresslookuptbl").flexOptions({
...
$(document).off('click',contentWB+' #voidwaybilltransactionmodal-savebtn:not(".disabled")').on('click',contentWB+' #voidwaybilltransactionmodal-savebtn:not(".disabled")',function(){
	...
	if(remarks.trim()==''){
		...
		$.confirm({
				...
				confirm: function(){
					$('#loading-img').removeClass('hidden');
					$.post(server+'waybill.php',{voidWaybillTransaction:'dROi$nsFpo94dnels$4sRoi809srbmouS@1!',waybillid:waybillid,waybillnumber:waybillnumber,remarks:remarks},function(data){
...
/*********************** MODAL BUTTONS *************************************/
$(document).off('click',contentWB+' #waybill-shipperpickupaddress-savebtn:not(".disabled")').on('click',contentWB+' #waybill-shipperpickupaddress-savebtn:not(".disabled")',function(){
	var modal = '#'+$(this).closest('.modal').attr('id'),
	...
	if(street==''){
		...
		$.post(server+'booking.php',{AddNewPickupAddress:'kkjO#@siaah3h0$09odfj$owenxezpo92po@k@',shipperid:shipperid,street:street,district:district,city:city,province:province,zipcode:zipcode,country:country},function(data){
			if(data.trim()=='success'){
				$(contentWB+" #waybill-shipperpickupaddresslookuptbl").flexOptions({
...
/********************* VIEWING *********************************/
$(document).off('click',contentWB+" .firstprevnextlastbtn button:not('.disabled')").on('click',contentWB+" .firstprevnextlastbtn button:not('.disabled')",function(){
	$('#loading-img').removeClass('hidden');
...
$(document).off('keyup',contentWB+' .transactionnumber').on('keyup',contentWB+' .transactionnumber',function(e){
	e.preventDefault();
	$('#loading-img').removeClass('hidden');
...
function getWaybillInformation(txnnumber){
	...
	$.post(server+'waybill.php',{getWaybillData:'F#@!3R3ksk#Op1NEi34smo1sonk&$',txnnumber:txnnumber},function(response){
		...
		if(response.trim()=='INVALID'){
			clearAllWB();
			$(contentWB+' .statusdiv').html('<br>');
			$(contentWB+' .billingflagdiv').html('<br>').removeClass('billed').removeClass('notbilled').removeAttr('billingflag');
			$(contentWB+" #pgtxnwaybill-id").val('').removeAttr('pgtxnwaybill-number','');
			$(contentWB+' .topbuttonsdiv').html("<div class='padded-with-border-engraved topbuttonswrapper'><div class='button-group'><div class='button-group-btn active' title='New' id='waybill-trans-newbtn'><img src='../resources/img/add.png'></div></div></div>");
			...
			$(contentWB+" #pgtxnwaybill-id").val(data["id"]).attr('pgtxnwaybill-number',txnnumber);
			...
			if(data["billedflag"]=='1'){
				$(contentWB+' .billingflagdiv').removeClass('notbilled').addClass('billed').html('BILLED').attr('billingflag',data["billedflag"]);
			}
			else{
				$(contentWB+' .billingflagdiv').removeClass('billed').addClass('notbilled').html('UNBILLED').attr('billingflag',data["billedflag"]);
			}
			...
			$(contentWB+" .waybill-freightcomputation").val(data["freightcomputation"]);
			$(contentWB+" .waybill-chargeableweight").val(data["chargeableweight"]).number(true,4);
			$(contentWB+" .waybill-freight").val(data["freight"]).number(true,decimalplaces);
			$(contentWB+" .waybill-valuation").val(data["valuation"]).number(true,decimalplaces);
			$(contentWB+" .waybill-insurancerate").val(data["insurancerate"]).number(true,decimalplaces);
			$(contentWB+" .waybill-fuelrate").val(data["fuelrate"]).number(true,decimalplaces);
			$(contentWB+" .waybill-bunkerrate").val(data["bunkerrate"]).number(true,decimalplaces);
			$(contentWB+" .waybill-minimumrate").val(data["minimumrate"]).number(true,decimalplaces);
			$(contentWB+" .waybill-regularcharges").val(data["totalregularcharges"]).number(true,decimalplaces);
			$(contentWB+" .waybill-otherchargesvatable").val(data["totalotherchargesvatable"]).number(true,decimalplaces);
			$(contentWB+" .waybill-otherchargesnonvatable").val(data["totalotherchargesnonvatable"]).number(true,decimalplaces);
			$(contentWB+" .waybill-subtotal").val(data["subtotal"]).number(true,decimalplaces);
			$(contentWB+" .waybill-vat").val(data["vat"]).number(true,decimalplaces);
			$(contentWB+" .waybill-totalamount").val(data["totalamount"]).number(true,decimalplaces);
			...
			$(inputfieldsWB+" .waybill-waybilltype").val(data["waybilltype"]).trigger('change');
			$(inputfieldsWB+" .waybill-rushflag").val(data["rushflag"]).trigger('change');
			$(inputfieldsWB+" .waybill-pulloutflag").val(data["pulloutflag"]).trigger('change');
			$(inputfieldsWB+" .waybill-odaflag").val(data["odaflag"]).trigger('change');

			if(data["odaflag"]==1){
				$(contentWB+' .odzflagdiv').removeClass('hidden').html('ODZ');
			}
			else{
				$(contentWB+' .odzflagdiv').addClass('hidden').html("<br>");
			}
			...
			if(data["bookingnumber"]!=null){
				$(inputfieldsWB+" .waybill-bookingnumber").empty().append('<option selected value="'+data["bookingnumber"]+'">'+data["bookingnumber"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-bookingnumber").empty().trigger('change');
			}

			if(data["thirdpartylogistic"]!=null){
				$(inputfieldsWB+" .waybill-3pl").empty().append('<option selected value="'+data["thirdpartylogisticid"]+'">'+data["thirdpartylogistic"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-3pl").empty().trigger('change');
			}
			if(data["agent"]!=null){
				$(inputfieldsWB+" .waybill-agent").empty().append('<option selected value="'+data["agentid"]+'">'+data["agent"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-agent").empty().trigger('change');
			}
			...
			if(data["destinationroute"]!=null){
				$(inputfieldsWB+" .waybill-destinationroute").empty().append('<option selected value="'+data["destinationrouteid"]+'">'+data["destinationroute"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-destinationroute").empty().trigger('change');
			}
			...
			if(data["deliveryinstruction"]!=null){
				$(inputfieldsWB+" .waybill-deliveryinstruction").empty().append('<option selected value="'+data["deliveryinstructionid"]+'">'+data["deliveryinstruction"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-deliveryinstruction").empty().trigger('change');
			}
			...
			if(data["transportcharges"]!=null){
				$(inputfieldsWB+" .waybill-transportcharges").empty().append('<option selected value="'+data["transportchargesid"]+'">'+data["transportcharges"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-transportcharges").empty().trigger('change');
			}

			if(data["carrier"]!=null){
				$(inputfieldsWB+" .waybill-carrier").empty().append('<option selected value="'+data["carrierid"]+'">'+data["carrier"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-carrier").empty().trigger('change');
			}

			if(data["pouchsize"]!=null){
				$(inputfieldsWB+" .waybill-pouchsize").empty().append('<option selected value="'+data["pouchsizeid"]+'">'+data["pouchsize"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-pouchsize").empty().trigger('change');
			}
			...
			if(data["expresstransactiontype"]!=null){
				$(inputfieldsWB+" .waybill-expresstransactiontype").empty().append('<option selected value="'+data["expresstransactiontype"]+'">'+data["expresstransactiontype"]+'</option>').trigger('change');
			}
			else{
				$(inputfieldsWB+" .waybill-expresstransactiontype").empty().trigger('change');
			}
			...
			if(data["status"]=="LOGGED"){
				...
				$(contentWB+" .topbuttonswrapper .button-group").html("<div class='button-group-btn active' title='New' id='waybill-trans-newbtn'><img src='../resources/img/add.png'></div>"+allowothertrans+"<div class='button-group-btn active' title='View Status History' id='waybill-trans-viewstatushistorybtn'><img src='../resources/img/history.png'></div>"+editcostingbtn+uploadwaybillstatusupdatebtn);
				//<div class='button-group-btn active' title='Upload' id='waybill-trans-uploadbtn'><img src='../resources/img/upload.png'></div>
				userAccess();
			}
				...
				$(contentWB+" .topbuttonswrapper .button-group").html("<div class='button-group-btn active' title='New' id='waybill-trans-newbtn'><img src='../resources/img/add.png'></div>"+allowothertrans+editbillingbtn+"<div class='button-group-btn active' title='Print' id='waybill-trans-printbtn'><img src='../resources/img/print.png'></div><div class='button-group-btn active' title='View Status History' id='waybill-trans-viewstatushistorybtn'><img src='../resources/img/history.png'></div>"+editchargesbtn+editcostingbtn+uploadwaybillstatusupdatebtn);
				//<div class='button-group-btn active' title='Upload' id='waybill-trans-uploadbtn'><img src='../resources/img/upload.png'></div>
				userAccess();
			}
				...
				$(contentWB+" .topbuttonswrapper .button-group").html("<div class='button-group-btn active' title='New' id='waybill-trans-newbtn'><img src='../resources/img/add.png'></div>"+allowothertrans+"<div class='button-group-btn active' title='Print' id='waybill-trans-printbtn'><img src='../resources/img/print.png'></div><div class='button-group-btn active' title='View Status History' id='waybill-trans-viewstatushistorybtn'><img src='../resources/img/history.png'></div>"+uploadwaybillstatusupdatebtn);
				//<div class='button-group-btn active' title='Upload' id='waybill-trans-uploadbtn'><img src='../resources/img/upload.png'></div>
				userAccess();
			}
			else{
			...
			$(contentWB+' #waybill-packagecodemodaltbl').DataTable().ajax.url("loadables/ajax/transactions.waybill-package-codes.php?waybill="+txnnumber).search('').load();
			if(data["hasaccess"]=='true'){
				$(contentWB+' .wbaddpackagecodebtnwrapper').html("<i class='fa fa-search inputgroupicon inputgroupbtnicon' alwaysshow title='View Package Code(s)' id='waybill-addpackagecodebtn' data-modal='#waybill-addpackagecodemodal'></i>");
			}
			else{
				$(contentWB+' .wbaddpackagecodebtnwrapper').empty();
			}
			...
			$.post(server+'waybill.php',{WaybillOtherChargesGetInfo:'kjoI$H2oiaah3h0$09jDppo92po@k@',txnnumber:txnnumber},function(dt){
		    	//alert(dt);
		    	rsp1 = $.parseJSON(dt);
		    	$(contentWB+' #waybill-otherchargestbl').DataTable().search('').clear().draw();
		    	for(var i=0;i<rsp1.length;i++){

		    		$(contentWB+' #waybill-otherchargestbl').DataTable().row.add([
		    ...
		    $.post(server+'waybill.php',{WaybillPackageDimensionsGetInfo:'kjoI$H2oiaah3h0$09jDppo92po@k@',txnnumber:txnnumber},function(dt2){
		    	//alert(dt);
		    	rsp2 = $.parseJSON(dt2);
		    	$(contentWB+' #waybill-packagedimensionsmodaltbl').DataTable().search('').clear().draw();
		    	for(var i=0;i<rsp2.length;i++){

		    		$(contentWB+' #waybill-packagedimensionsmodaltbl').DataTable().row.add([
							"<input type='checkbox' class='rowcheckbox hidden'>",
		    ...
		    $.post(server+'waybill.php',{getHandlingInstructions:'sdfed#n2L1hfi$n#opi3opod30napri',txnnumber:txnnumber},function(data){
				...
				for (var i = 0; i < instructions.length; i++) {
					...
					if(otherwhse1[1]!=null){
						$(contentWB+' .waybill-handlinginstruction').append('<option selected value="'+otherwhse1[0]+'">'+otherwhse1[1]+'</option>').trigger('change');
					}
					else{
						$(contentWB+' .waybill-handlinginstruction').empty().trigger('change');
					}
			...
			$.post(server+'waybill.php',{getAccompanyingDocuments:'sdfed#n2L1hfi$n#opi3opod30napri',txnnumber:txnnumber},function(data){
				...
				for (var i = 0; i < descriptions.length; i++) {
					...
					if(strdesc[1]!=null){
						$(contentWB+" .waybill-document").append('<option selected value="'+strdesc[0]+'">'+strdesc[1]+'</option>').trigger('change');
					}
					else{
			...
			if(data["fieldeditwbcbm"]=='true'){
					$(contentWB+' .cbmwrapper').find(".control-label").html('CBM <img class="controllabeliconedit" id="fieldedit-wbcbm" data-label="CBM" data-code="WBCBM" data-table="txn_waybill" data-column="package_cbm" data-txncolumn="waybill_number" data-txnnumber="'+txnnumber+'" data-type="numeric" src="../resources/img/editinputicon.png">');
			}
			else{
					$(contentWB+' .cbmwrapper').find(".control-label").html('CBM');
			}
			 /****** FIELD EDIT ****************/
...
$(document).off('dblclick',contentWB+' .waybillsearchrow').on('dblclick',contentWB+' .waybillsearchrow',function(){
	var modal = '#'+$(this).closest('.modal').attr('id');
...
$(document).on('keyup',contentWB+" #waybill-searchmodallookup .waybillsearch-pickupdatefrom,"+contentWB+" #waybill-searchmodallookup .waybillsearch-pickupdateto, "+contentWB+" #waybill-searchmodallookup .waybillsearch-bookingnumber, "+contentWB+" #waybill-searchmodallookup .waybillsearch-manifestnumber, "+contentWB+" #waybill-searchmodallookup .waybillsearch-invoicenumber, #waybill-searchmodallookup .waybillsearch-trackingnumber, #waybill-searchmodallookup .waybillsearch-mawbl",function(e){
	var key = e.which||e.keycode;
	if(key=='13'){
		var modal = "#"+$(this).closest('.modal').attr('id');
...
$(document).off('click',contentWB+' #waybillsearch-searchbtn:not(".disabled")').on('click',contentWB+' #waybillsearch-searchbtn:not(".disabled")',function(){
...
$(document).off('click',contentWB+' #waybill-trans-printbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-printbtn:not(".disabled")',function(){
	...
	$(document).off('shown.bs.modal',modal).on('shown.bs.modal',modal,function(){
			$(document).off('shown.bs.modal',modal);
			$(modal+' .waybillprintingmodal-bolnumber').focus().select();
			btn.removeClass('disabled');
	});

});

$(document).off('click',contentWB+' #waybillprintingmodal-savebtn:not(".disabled")').on('click',contentWB+' #waybillprintingmodal-savebtn:not(".disabled")',function(){
	var modal = '#'+$(this).closest('.modal').attr('id');
	var bolnumber = $(modal+' .waybillprintingmodal-bolnumber').val();
	var formtype = $(modal+' .waybillprintingmodal-formtype').val();
	var remarks = $(modal+' .waybillprintingmodal-remarks').val().replace(/\s/g,'%20');
			...
			if(formtype=='INTERNAL'||formtype=='INTERNAL-ALT'){
									...
									if(data.trim()=='success'){
										...
										$(document).off('hidden.bs.modal',modal).on('hidden.bs.modal',modal,function(){
											...
											if($(".content>.content-tab-pane .content-tabs").find("li[data-pane='#"+tabid+"tabpane']").length>=1){
												...
												$('.content>.content-tab-pane .content-pane-wrapper>.content-pane:last-child').load("Printouts/print-preview.php?txnnumber="+waybillnumber+"&source=printouts/transactions/waybill.php?txnnumber="+waybillnumber+'tpicorpaabbccreference='+waybillnumber+"tpicorpaabbccbolnumber="+bolnumber+"tpicorpaabbccformtype="+formtype+"tpicorpaabbccremarks="+remarks);
												setTimeout(function(){
													$('#loading-img').addClass('hidden');
												},400);
											}
				...
				$(document).off('hidden.bs.modal',modal).on('hidden.bs.modal',modal,function(){
						...
						if($(".content>.content-tab-pane .content-tabs").find("li[data-pane='#"+tabid+"tabpane']").length>=1){
							...
							$('.content>.content-tab-pane .content-tabs').append("<li data-pane='#"+tabid+"tabpane' class='active'>"+title+"<i class='fa fa-remove'></i></li>");
							$('.content>.content-tab-pane .content-pane-wrapper').append("<div class='content-pane active' id='"+tabid+"tabpane'></div>");
							$('.content>.content-tab-pane .content-pane-wrapper>.content-pane:last-child').load("Printouts/print-preview.php?txnnumber="+waybillnumber+"&source=printouts/transactions/waybill-external-form.php?txnnumber="+waybillnumber+'tpicorpaabbccreference='+waybillnumber+'tpicorpaabbccbolnumber='+bolnumber+"tpicorpaabbccformtype="+formtype+"tpicorpaabbccremarks="+remarks);
							setTimeout(function(){
								$('#loading-img').addClass('hidden');
							},400);
						}
				...
				$(document).off('hidden.bs.modal',modal).on('hidden.bs.modal',modal,function(){
						...
						if($(".content>.content-tab-pane .content-tabs").find("li[data-pane='#"+tabid+"tabpane']").length>=1){
							...
							$('.content>.content-tab-pane .content-tabs').append("<li data-pane='#"+tabid+"tabpane' class='active'>"+title+"<i class='fa fa-remove'></i></li>");
							$('.content>.content-tab-pane .content-pane-wrapper').append("<div class='content-pane active' id='"+tabid+"tabpane'></div>");
							$('.content>.content-tab-pane .content-pane-wrapper>.content-pane:last-child').load("Printouts/print-preview.php?txnnumber="+waybillnumber+"&source=printouts/transactions/waybill-dr-form.php?txnnumber="+waybillnumber+'tpicorpaabbccreference='+waybillnumber+'tpicorpaabbccbolnumber='+bolnumber+"tpicorpaabbccformtype="+formtype);
							setTimeout(function(){
								$('#loading-img').addClass('hidden');
							},400);
						}
...
$(document).off('click',contentWB+' #waybill-trans-updatestatusbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-updatestatusbtn:not(".disabled")',function(){
...
$(document).off('click',contentWB+' #waybill-updatestatusmodal-savebtn:not(".disabled")').on('click',contentWB+' #waybill-updatestatusmodal-savebtn:not(".disabled")',function(){
	...
	if(status==''||status==null||status=='null'||status=='NULL'||status==undefined){
	...
	else if((status.toUpperCase()=='DEL'||status.toUpperCase()=='DELIVERED'||status.toUpperCase()=='RTS'||status.toUpperCase()=='RETURN TO SHIPPER')&&receiveddate.trim()==''){
	...
	else if((status.toUpperCase()=='DEL'||status.toUpperCase()=='DELIVERED'||status.toUpperCase()=='RTS'||status.toUpperCase()=='RETURN TO SHIPPER')&&(personnel==null||personnel=='null'||personnel=='NULL'||personnel==undefined)&&tpl==1){
		...
		$.confirm({
				...
				confirm: function(){
					$('#loading-img').removeClass('hidden');
					$.post(server+'waybill.php',{updateStatusWaybill:'kkjO#@siaah3h0$09odfj$owenxezpo92po@k@',waybillid:waybillid,status:status,remarks:remarks,receivedby:receivedby,receiveddate:receiveddate,personnel:personnel},function(data){
						...
						if(data.trim()=='success'){
							...
							$(document).off('hidden.bs.modal',modal).on('hidden.bs.modal',modal,function(){
								...
								$(modal+' .waybill-updatestatusmodal-status').empty().trigger('change');
								$(modal+' .waybill-updatestatusmodal-personnel').empty().trigger('change');
...
$(document).off('click',contentWB+' #waybill-trans-viewstatushistorybtn:not(".disabled")').on('click',contentWB+' #waybill-trans-viewstatushistorybtn:not(".disabled")',function(){
	$(contentWB+' #waybill-statushistorymodal').modal('show');

	var waybillnumber = $(contentWB+' #pgtxnwaybill-id').attr('pgtxnwaybill-number');
	

	$(document).off('shown.bs.modal',contentWB+' #waybill-statushistorymodal').on('shown.bs.modal',contentWB+' #waybill-statushistorymodal',function(){

			$(document).off('shown.bs.modal',contentWB+' #waybill-statushistorymodal');
			$(contentWB+" #waybill-statushistorytbl").flexOptions({
					url:'loadables/ajax/transactions.waybill.status-history.php?waybillnumber='+waybillnumber,
					sortname: "created_date",
					sortorder: "desc"
			}).flexReload(); 

			$(contentWB+" #waybill-billinghistorytbl").flexOptions({
...
$(document).off('keyup',contentWB+' .packagedimensionsmodalflds-length').on('keyup',contentWB+' .packagedimensionsmodalflds-length',function(){

	computeWBPDWolumetricWeight();
	computeWBPDcbm();
});
$(document).off('keyup',contentWB+' .packagedimensionsmodalflds-width').on('keyup',contentWB+' .packagedimensionsmodalflds-width',function(){

	computeWBPDWolumetricWeight();
	computeWBPDcbm();
});
$(document).off('keyup',contentWB+' .packagedimensionsmodalflds-height').on('keyup',contentWB+' .packagedimensionsmodalflds-height',function(){

	computeWBPDWolumetricWeight();
	computeWBPDcbm();
});
$(document).off('keyup',contentWB+' .packagedimensionsmodalflds-quantity').on('keyup',contentWB+' .packagedimensionsmodalflds-quantity',function(){
...
function waybillPackageDimensionInsertRow(table){
	var length = $(contentWB+' .packagedimensionsmodalflds-length').val(),
		...
		/*else if(!length>0){
			...
			$(contentWB+' .packagedimensionsmodalflds-quantity').focus().select();
		}
		else if(uom==null||uom==''||uom==undefined||uom=='NULL'||uom=='null'){
			$(contentWB+' .packagedimensionsmodalflds-uom').select2('open');
		}
		else if(!(parseFloat(actualweight)>0)){
			$(contentWB+' .packagedimensionsmodalflds-actualweight').focus().select();
		}
		else if(!(parseFloat(cbm)>0)){
			$(contentWB+' .packagedimensionsmodalflds-cbm').focus().select();
		}
		else if(!(parseFloat(volweight)>0)){
			$(contentWB+' .packagedimensionsmodalflds-volweight').focus().select();
		}
...
$(document).off('click',contentWB+' .packagedimensionsmodalflds-insertbtn').on('click',contentWB+' .packagedimensionsmodalflds-insertbtn',function(){
...
$(document).off('click',contentWB+' .packagedimensionsmodalflds-removebtn').on('click',contentWB+' .packagedimensionsmodalflds-removebtn',function(){
...
function waybillPackageDimensionClearFields(){
		...
		$(contentWB+' .packagedimensionsmodalflds-quantity').val('').focus().select();
		$(contentWB+' .packagedimensionsmodalflds-volweight').val('');
		$(contentWB+' .packagedimensionsmodalflds-cbm').val('');
		$(contentWB+' .packagedimensionsmodalflds-uom').empty().trigger('change');
		$(contentWB+' .packagedimensionsmodalflds-actualweight').val('');
}

$(document).off('click',contentWB+' .packagedimensionsmodalflds-clearbtn ').on('click',contentWB+' .packagedimensionsmodalflds-clearbtn',function(){
		waybillPackageDimensionClearFields();
});
////// CLEAR RATE INPUT FIELDS END
...
function wbaddpackagecode(modal){
	...
	if(code.trim()==''){
		button.removeClass('disabled');
		$(modal+' .addpackagecodemodal-code').val('').focus();
	}
	else{
		$.post(server+'waybill.php',{AddWaybillPackageCode:'#@1F4fdgrw$ghjt3K@3#4hh$9v7&3s',waybillnumber:waybillnumber,code:code},function(data){
			...
			if(data.trim()=='success'){
				button.removeClass('disabled');
				$(contentWB+' #waybill-packagecodemodaltbl').DataTable().ajax.url("loadables/ajax/transactions.waybill-package-codes.php?waybill="+waybillnumber).search('').load();
				$(modal+' .addpackagecodemodal-code').val('').focus();
...
$(document).off('click',contentWB+' .addpackagecodemodal-insertbtn:not(".disabled")').on('click',contentWB+' .addpackagecodemodal-insertbtn:not(".disabled")',function(){
	var modal = '#'+$(this).closest('.modal').attr('id');
	wbaddpackagecode(modal);
});

$(document).off('keyup',contentWB+' .addpackagecodemodal-code').on('keyup',contentWB+' .addpackagecodemodal-code',function(e){
	var modal = '#'+$(this).closest('.modal').attr('id');
...
$(document).off('click',contentWB+' .addpackagecodemodal-removebtn:not(".disabled")').on('click',contentWB+' .addpackagecodemodal-removebtn:not(".disabled")',function(){
...
$(document).off('click',contentWB+' #waybill-trans-uploadbtn').on('click',contentWB+' #waybill-trans-uploadbtn',function(){
...
$(document).off('click',contentWB+' #waybill-uploadtransactionfilemodal-uploadbtn:not(".disabled")').on('click',contentWB+' #waybill-uploadtransactionfilemodal-uploadbtn:not(".disabled")',function(){
	...
	if($(contentWB+' '+modal+' .waybilluploadtransactionfile').val().trim()==''){
		...
		$(modal).modal('hide');
		$(document).off('hidden.bs.modal',contentWB+' '+modal).on('hidden.bs.modal',contentWB+' '+modal,function(){
...
$(document).off('change',contentWB+' .togglebillingflagmodal-billingflag').on('change',contentWB+' .togglebillingflagmodal-billingflag',function(){
...
$(document).off('click',contentWB+' #waybill-trans-billingtogglebtn:not(".disabled")').on('click',contentWB+' #waybill-trans-billingtogglebtn:not(".disabled")',function(){
	...
	$(contentWB+' .togglebillingflagmodal-billingflag').val(billingflag).trigger('change');
...
$(document).off('click',contentWB+' #togglebillingflagmodal-savebtn:not(".disabled")').on('click',contentWB+' #togglebillingflagmodal-savebtn:not(".disabled")',function(){
	...
	if(billingflag==1&&billingreference.trim()==''){
		...
		$(modal+' .modal-errordiv').html("<div class='message'><div class='message-content'><span class='closemessage'>&times;</span>Please provide reason for changing billing flag.</div></div>");
		...
		$.confirm({
				...
				confirm: function(){
					$('#loading-img').removeClass('hidden');
					$.post(server+'waybill.php',{changeBillingFlagging:'dROi$nsFpo94dnels$4sRoi809srbmouS@1!',txnid:txnid,txnnumber:txnnumber,remarks:remarks,billingflag:billingflag,billingreference:billingreference},function(data){
						...
						if(data.trim()=='success'){
							...
							$(document).off('hidden.bs.modal',modal).on('hidden.bs.modal',modal,function(){
								...
								$(modal+' .togglebillingflagmodal-billingflag').val(0).trigger('change');
...
$(document).off('change',contentWB+' .waybill-updatestatusmodal-status').on('change',contentWB+' .waybill-updatestatusmodal-status',function(){

	var status = $(this).val();
	var modal = '#'+$(this).closest('.modal').attr('id');

	if(status=='DEL'||status=='DELIVERED'||status=='del'||status=='delivered'||status=='RTS'||status=='RETURN TO SHIPPER'||status=='rts'||status=='return to shipper'){
...
$(document).off('click',contentWB+' .deletewaybillstatushistorybtn:not(".disabled")').on('click',contentWB+' .deletewaybillstatushistorybtn:not(".disabled")',function(){
...
$(document).off('click',contentWB+' #deletewaybillstatushistorymodal-deletebtn:not(".disabled")').on('click',contentWB+' #deletewaybillstatushistorymodal-deletebtn:not(".disabled")',function(){
...
$(document).off('click',contentWB+' #waybill-trans-editwaybillchargesbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-editwaybillchargesbtn:not(".disabled")',function(){
	...
	$.post(server+'waybill.php',{getWaybillData:'F#@!3R3ksk#Op1NEi34smo1sonk&$',txnnumber:waybillnumber},function(response){
		...
		if(response.trim()=='INVALID'){
			...
			if(data["waybilltype"]=='PARCEL'){
				$(contentWB+" .editwaybillchargesmodal-fixedrateamount").closest('.form-group').removeClass('hidden');
				$(contentWB+" .editwaybillchargesmodal-totalhandlingcharges").closest('.form-group').removeClass('hidden');
				$(contentWB+" .editwaybillchargesmodal-insurancerate").closest('.form-group').removeClass('hidden');
			}
			else{
				$(contentWB+" .editwaybillchargesmodal-fixedrateamount").val(0).closest('.form-group').addClass('hidden');
				$(contentWB+" .editwaybillchargesmodal-totalhandlingcharges").val(0).closest('.form-group').addClass('hidden');
				$(contentWB+" .editwaybillchargesmodal-insurancerate").val(0).closest('.form-group').addClass('hidden');
			...
			$.post(server+'waybill.php',{WaybillOtherChargesGetInfo:'kjoI$H2oiaah3h0$09jDppo92po@k@',txnnumber:waybillnumber},function(dt){
		    	rsp1 = $.parseJSON(dt);
		    	$(contentWB+' #editwaybillchargesmodal-otherchargestbl').DataTable().search('').clear().draw();
		    	for(var i=0;i<rsp1.length;i++){

		    		$(contentWB+' #editwaybillchargesmodal-otherchargestbl').DataTable().row.add([
							"<input type='checkbox' class='rowcheckbox'>",
		    ...
		    $.post(server+'waybill.php',{getHandlingInstructions:'sdfed#n2L1hfi$n#opi3opod30napri',txnnumber:waybillnumber},function(data){
				...
				for (var i = 0; i < instructions.length; i++) {
					var otherwhse1 = instructions[i];
					    otherwhse1 = otherwhse1.split('%$&');
					if(otherwhse1[1]!=null){
						$(contentWB+' .editwaybillchargesmodal-handlinginstruction').append('<option selected value="'+otherwhse1[0]+'">'+otherwhse1[1]+'</option>').trigger('change');
					}
					else{
						$(contentWB+' .editwaybillchargesmodal-handlinginstruction').empty().trigger('change');
					}
...
$(document).off('click',contentWB+' .editwaybillchargesmodal-otherchargesinsertbtn').on('click',contentWB+' .editwaybillchargesmodal-otherchargesinsertbtn',function(){
...
$(document).off('click',contentWB+' .editwaybillchargesmodal-otherchargesremovebtn').on('click',contentWB+' .editwaybillchargesmodal-otherchargesremovebtn',function(){
...
function editWaybillChargesOtherChargesClearFields(){
	    $(contentWB+' .editwaybillchargesmodal-otherchargesamount').val('').focus();
	    $(contentWB+' .editwaybillchargesmodal-otherchargesdesc').empty().trigger('change');
}

$(document).off('click',contentWB+' .editwaybillchargesmodal-otherchargesclearbtn').on('click',contentWB+' .editwaybillchargesmodal-otherchargesclearbtn',function(){
		editWaybillChargesOtherChargesClearFields();
});
////// CLEAR RATE INPUT FIELDS END - EDIT WAYBILL CHARGES


$(document).off('keyup','#editwaybillchargesmodal .ratefld').on('keyup','#editwaybillchargesmodal .ratefld',function(){
...
$(document).off('keyup','#editwaybillchargesmodal .editwaybillchargesmodal-fixedrateamount').on('keyup','#editwaybillchargesmodal .editwaybillchargesmodal-fixedrateamount',function(){
...
$(document).off('click','#editwaybillchargesmodal #editwaybillchargesmodal-savebtn:not(".disabled")').on('click','#editwaybillchargesmodal #editwaybillchargesmodal-savebtn:not(".disabled")',function(){
	...
	if(returndocumentfee<0||returndocumentfee==null||returndocumentfee==undefined||isNaN(returndocumentfee)==1){
		...
		$.post(server+'waybill.php',{ConfirmEditedCharges:'D#@ihQnsRPFG%$po92po@k@',waybillnumber:waybillnumber,returndocumentfee:returndocumentfee,waybillfee:waybillfee,securityfee:securityfee,docstampfee:docstampfee,oda:oda,valuation:valuation,freight:freight,insurancerate:insurancerate,fuelrate:fuelrate,bunkerrate:bunkerrate,totalhandlingcharges:totalhandlingcharges,zeroratedflag:zeroratedflag,handlinginstruction:handlinginstruction,linedesc:linedesc,lineamount:lineamount,linevatflag:linevatflag,totalregularcharges:totalregularcharges,totalotherchargesvatable:totalotherchargesvatable,subtotal:subtotal,vat:vat,totalotherchargesnonvatable:totalotherchargesnonvatable,totalamount:totalamount,fixedrateamount:fixedrateamount,chargeableweight:chargeableweight},function(data){
...
$(document).off('click',contentWB+' #waybill-trans-resetprintcounterbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-resetprintcounterbtn:not(".disabled")',function(){
...
$(document).off('click',contentWB+' #resetprintcountermodal-savebtn:not(".disabled")').on('click',contentWB+' #resetprintcountermodal-savebtn:not(".disabled")',function(){
	var modal = '#'+$(this).closest('.modal').attr('id');
	var btn = $(this);
	    btn.addClass('disabled');
	var remarks = $(modal+' .resetprintcountermodal-remarks').val().trim();
...
$(document).off('click',contentWB+' #waybill-packagedimensionsbtn:not(".disabled")').on('click',contentWB+' #waybill-packagedimensionsbtn:not(".disabled")',function(){
...
$(document).off('select2:select',contentWB+' .waybill-modeoftransport').on('select2:select',contentWB+' .waybill-modeoftransport',function(e){
...
$(document).off('click',contentWB+' #waybill-trans-editwaybillcostingbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-editwaybillcostingbtn:not(".disabled")',function(){
			...
			$.post(server+'waybill.php',{getWaybillData:'F#@!3R3ksk#Op1NEi34smo1sonk&$',txnnumber:txnnumber},function(response){
				...
				if(response.trim()=='INVALID'){
					...
					$(document).off('shown.bs.modal',modal).on('shown.bs.modal',modal,function(){
							...
							$(contentWB+" .editwaybillcostingmodal-billreference").val(data["billreference"]).focus().select();
					});
...
$(document).off('click',contentWB+' #editwaybillcostingmodal-savebtn:not(".disabled")').on('click',contentWB+' #editwaybillcostingmodal-savebtn:not(".disabled")',function(){
	...
	if(freightcost!=''&&freightcost<0){
		...
		$(contentWB+' .editwaybillcostingmodal-freightcost').focus().select();
		btn.removeClass('disabled');
	}
	else if(agentcost!=''&&agentcost<0){
		$(modal+' .modal-errordiv').html("<div class='message'><div class='message-content'><span class='closemessage'>&times;</span>Invalid Freight Cost</div></div>");
		$(contentWB+' .editwaybillcostingmodal-agentcost').focus().select();
		btn.removeClass('disabled');
	}
	else if(insuranceamount!=''&&insuranceamount<0){
		$(modal+' .modal-errordiv').html("<div class='message'><div class='message-content'><span class='closemessage'>&times;</span>Invalid Freight Cost</div></div>");
		$(contentWB+' .editwaybillcostingmodal-insuranceamount').focus().select();
		btn.removeClass('disabled');
	}
	else{
		$.post(server+'waybill.php',{saveWaybillCostingDetails:'F#@!3R3ksk#Op1NEi34smo1sonk&$',txnnumber:txnnumber,txnid:txnid,billreference:billreference,freightcost:freightcost,agentcost:agentcost,billitemnumber:billitemnumber,insuranceamount:insuranceamount,insurancereference:insurancereference},function(response){
...
/************* EDIT DIMENSIONS BTN *********************/
$(document).off('click',contentWB+' #waybill-trans-editdimensionsbtn:not(".disabled")').on('click',contentWB+' #waybill-trans-editdimensionsbtn:not(".disabled")',function(){
	var button = $(this);
	button.addClass('disabled');
	...
	$.post(server+'waybill.php',{checkWaybillEnableEdit:'F#@!3R3ksk#Op1NEi34smo1sonk&$',wbnumber:wbnumber},function(data){
			...
			if(rp['response']=='success'){
				...
				$(contentWB+' .packagedimensionsmodalflds').removeClass('hidden');
				$(contentWB+' .wbaddpackagecodebtnwrapper').empty();
				$(contentWB+' .waybill-vwcbm, '+contentWB+' .waybill-vw, '+contentWB+' .waybill-mawbl, '+contentWB+' .waybill-actualweight, '+contentWB+' .waybill-pickupdate, '+contentWB+' .waybill-remarks').removeAttr('disabled');
...
$(document).off('click',contentWB+' #waybill-trans-uploadwaybillstatusupdatebtn').on('click',contentWB+' #waybill-trans-uploadwaybillstatusupdatebtn',function(){
		var modal = '#uploadwaybillstatusupdatemodal';
		$(modal).modal('show');
});


$(document).off('click',contentWB+' #uploadwaybillstatusupdatemodal-uploadbtn:not(".disabled")').on('click',contentWB+' #uploadwaybillstatusupdatemodal-uploadbtn:not(".disabled")',function(){
	...
	if($(contentWB+' '+modal+' .uploadwaybillstatusupdatemodal-file').val().trim()==''){
		say('Please select a file to upload');
		button.removeClass('disabled');
	}
	else{
...
$(document).off('click',contentWB+' #waybill-consigneelookup-savebtn:not(".disabled")').on('click',contentWB+' #waybill-consigneelookup-savebtn:not(".disabled")',function(){
	var modal = '#'+$(this).closest('.modal').attr('id'),
	...
	if(accountname==''){
		...
		$.post(server+'waybill.php',{AddNewConsignee:'kkjO#@siaah3h0$09odfj$owenxezpo92po@k@',accountname:accountname,companyname:companyname,contact:contact,tel:tel,mobile:mobile,street:street,district:district,city:city,region:region,zipcode:zipcode,country:country},function(data){
			var rsp = data.split("@#$%&");
			
			if(rsp[0].trim()=='success'){
				$(contentWB+" #waybill-consigneelookuptbl").flexOptions({
...
$(document).off('show.bs.modal',contentWB+' #waybill-consigneelookup').on('show.bs.modal',contentWB+' #waybill-consigneelookup',function(){
	...
	$.post(server+'waybill.php',{checkIfUserCanAddNewConsignee:'dROi$nsFpo94dnels$4sRoi809srbmouS@1!'},function(data){
			if(data.trim()=='1'){
				$(contentWB+" .waybill-consigneelookup-country").empty().append('<option selected value="Philippines">Philippines</option>').trigger('change');
...
$(document).off('focus',contentWB+' .select2').on('focus',contentWB+' .select2',function(e){
...
$(document).off('keydown',contentWB+' .waybill-shipmentdescription').on('keydown',contentWB+' .waybill-shipmentdescription',function(e){
	var key = e.keycode||e.which;
	if(key=='9'){
...
$(document).off('keydown',contentWB+' .waybill-shipper-telephone').on('keydown',contentWB+' .waybill-shipper-telephone',function(e){
	var key = e.keycode||e.which;
	if(key=='9'){
...
$(document).off('shown.bs.modal',contentWB+' .modal').on('shown.bs.modal',contentWB+' .modal',function(){
	$(this).find(".qsbox").focus().select();
});

$(document).off('hidden.bs.modal',contentWB+' #waybill-packagedimensionsmodal').on('hidden.bs.modal',contentWB+' #waybill-packagedimensionsmodal',function(){
	$(inputfieldsWB+' .waybill-declaredvalue').focus().select();
});


$(document).off('shown.bs.modal',contentWB+' #waybill-packagedimensionsmodal').on('shown.bs.modal',contentWB+' #waybill-packagedimensionsmodal',function(){
...
$(document).off("select2:close",contentWB+' .waybill-modeoftransport').on("select2:close",contentWB+' .waybill-modeoftransport',function(e) { 
	if($(contentWB+' .waybill-waybilltype').val()=='PARCEL'){
...
$(document).off("click",contentWB+" #waybillprintingmodal-printbtn").on("click",contentWB+" #waybillprintingmodal-printbtn",function(){
		...
		say("Please select form type");
			...
			if(formtype=='INTERNAL'||formtype=='INTERNAL-ALT'){
								...
								$.post(server+'waybill.php',{increaseprintcounter:'KFHoEO#0HELKN#Opsy#lka$P#HlNlk!I#H$',waybillid:waybillid},function(data){
									...
									if(data.trim()=='success'){
										...
										$(document).off('hidden.bs.modal',modal).on('hidden.bs.modal',modal,function(){
											...
											$(contentWB+' #WBprintpdfiframe').attr('src',"printouts/transactions/waybill.php?txnnumber="+waybillnumber+"&bolnumber="+bolnumber+"&formtype="+formtype+"&remarks="+remarks);
				...
				$(document).off('hidden.bs.modal',modal).on('hidden.bs.modal',modal,function(){
						...
						$(contentWB+' #WBprintpdfiframe').attr('src',"printouts/transactions/waybill-external-form.php?txnnumber="+waybillnumber+"&bolnumber="+bolnumber+"&formtype="+formtype+"&remarks="+remarks);
...
$(document).off("change",contentWB+' .waybill-pickupdate').on("change",contentWB+' .waybill-pickupdate',function() { 
        $(contentWB+' .waybill-waybilltype').select2('open'); 
});



$(document).off("change",contentWB+' .uploadwaybillstatusupdatemodal-type').on("change",contentWB+' .uploadwaybillstatusupdatemodal-type',function() { 
