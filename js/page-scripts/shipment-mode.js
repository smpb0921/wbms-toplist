$(document).off('click','.editshipmentmodebtn').on('click','.editshipmentmodebtn',function(){
	var modal = "#editshipmentmodemodal";
	$(modal+' .code').val($(this).attr('code'));
	$(modal+' .description').val($(this).attr('desc'));
	$(modal+' .shipmentmodemodalid').val($(this).attr('rowid'));
});

/***************************** SAVE | EDIT **************************************************/
$(document).off('click','.shipmentmodemodal-savebtn:not(".disabled")').on('click','.shipmentmodemodal-savebtn:not(".disabled")',function(){
	...
	if(modal=='#editshipmentmodemodal'){
		id = $(modal+' .shipmentmodemodalid').val();
		source = 'edit';
	...
	if(code==''){
		...
		$.post(server+'shipment-mode.php',{shipmentmodeSaveEdit:'j$isnDPo#P3sll3p23a3!@3kzlsO!mslo#k@',source:source,id:id,code:code,desc:desc},function(data){
				if(data.trim()=='success'){
					$(modal).modal('hide');
					$(modal).on('hidden.bs.modal',function(){
						
						$('#shipmentmodetable').flexOptions({
											url:'loadables/ajax/maintenance.shipment-mode.php',
											sortname: newsort,
											sortorder: "desc"
						}).flexReload(); 
