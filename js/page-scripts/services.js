$(document).off('click','.editservicesbtn').on('click','.editservicesbtn',function(){
	var modal = "#editservicesmodal";
	$(modal+' .code').val($(this).attr('code'));
	$(modal+' .description').val($(this).attr('desc'));
	$(modal+' .servicesmodalid').val($(this).attr('rowid'));
});

/***************************** SAVE | EDIT **************************************************/
$(document).off('click','.servicesmodal-savebtn:not(".disabled")').on('click','.servicesmodal-savebtn:not(".disabled")',function(){
	...
	if(modal=='#editservicesmodal'){
		id = $(modal+' .servicesmodalid').val();
		source = 'edit';
	...
	if(code==''){
		...
		$.post(server+'services.php',{ServicesSaveEdit:'j$isnDPo#P3sll3p23a3!@3kzlsO!mslo#k@',source:source,id:id,code:code,desc:desc},function(data){
