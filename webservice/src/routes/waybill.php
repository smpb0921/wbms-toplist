	...
	try {
		...
		if($rowcount==1||1==1){
				...
				$sql = "SELECT 	txn_waybill_status_history.status_code,
						...
						from ( SELECT * FROM txn_waybill
						where (
							txn_waybill.waybill_number=:waybill OR (txn_waybill.mawbl_bl = :waybill and txn_waybill.mawbl_bl is not null))   
							) txn_waybill
						left join txn_waybill_status_history  on txn_waybill.waybill_number=txn_waybill_status_history.waybill_number
						...
						where txn_waybill_status_history.status_description in (select status from webapi_status_shown)

						order by txn_waybill.waybill_number,txn_waybill_status_history.created_date asc";
				...
				if($rowcount>0){
					while($obj=$stmt->fetch(PDO::FETCH_OBJ)){
						...
						$temp = array(
										"date"=>utf8_encode(dateFormat($obj->created_date,'m/d/Y g:i A')),
										"movedate"=>utf8_encode(dateFormat($obj->created_date,'d M Y')),
										"movetime"=>utf8_encode(dateFormat($obj->created_date,'H:i')),
										"received_date"=>utf8_encode(dateFormat($obj->received_date,'m/d/Y')),
										"received_by"=>utf8_encode($obj->received_by),
					...
					if($rowcount==1){
						...
						return $response->withStatus(400)->withHeader('Content-Type', 'application/json')->write(json_encode([
						    "Success" => false,
						    "Message" => "Invalid Waybill Number : <b>{$wbnumber}</b>"
							]));
